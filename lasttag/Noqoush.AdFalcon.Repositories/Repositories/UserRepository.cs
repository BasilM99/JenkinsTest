...
namespace Noqoush.AdFalcon.Persistence.Repositories
{
    public class UserRepository : RepositoryBase<User, int>, IUserRepository
    {
        ...
        public IEnumerable<User> GetSSPPartners(AllAppSiteCriteria criteria, out int Count)
        {
         ...
         //   rootQuery.JoinAlias(() => userAlias.Account, () => AccountAlias);
            var AppSitesList = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.AppSite.AppSite>().Select(M => M.Account.ID)
           .Where(M => M.Account.ID == AccountAlias.ID);
            AppSitesList.Where(m => m.IsDeleted == false);

            var PrimaryUsers = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Account.Account>().Select(M => M.PrimaryUser.ID)
       ...
       .Where(M => M.Account.ID == AccountAlias.ID);
            ...
            {
                Noqoush.AdFalcon.Domain.Model.AppSite.SubAppsite SubAppsiteAlias = null;
...

            {
                ...
                Noqoush.AdFalcon.Domain.Model.Account.Account AccountAppSIteAlias = null;
                Noqoush.AdFalcon.Domain.Model.Account.User UserAccountAppSIteAlias = null;
    ...
    .Select(gm => gm.ID);
            ...
            if (criteria.Role>0)
            {
                var subqueryRoleUser = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Account.Account>()
    ...
    .Select(gm => gm.ID);
      ...
      /*  public Noqoush.AdFalcon.Domain.Model.Account.Account GetUserAccountByEmail(string emailAddress)
        {
            ISession nhibernateSession = UnitOfWork.Current.OrmSession<ISession>();
            User userAlias = null;
            ...
            return rootQuery.SingleOrDefault<Noqoush.AdFalcon.Domain.Model.Account.Account>();
        } */
    }
}
