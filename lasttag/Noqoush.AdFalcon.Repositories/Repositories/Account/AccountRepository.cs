...
{
    public class AccountRepository : RepositoryBase<Noqoush.AdFalcon.Domain.Model.Account.Account, int>, IAccountRepository
    {
        ...
        public AccountRepository(RepositoryImplBase<Noqoush.AdFalcon.Domain.Model.Account.Account, int> repository)
            : base(repository)
        {
     ...
     //.Where(M => M.ID == AuditTrialAlias.ObjectType.ID).Where(M => M.RootID == filter.ObjectTypeID);
        ...
        //    }
            ...
            if (!string.IsNullOrEmpty(filter.UserName))
            {
                ...
                disjunction.Add(Restrictions.Where < Noqoush.AdFalcon.Domain.Model.Account.User > (M=>M.FirstName.IsLike(filter.UserName,MatchMode.Anywhere)));


                disjunction.Add(Restrictions.Where<Noqoush.AdFalcon.Domain.Model.Account.User>(M => M.LastName.IsLike(filter.UserName, MatchMode.Anywhere)));
                var UserObjectIdSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Account.User>().Select(M => M.ID)
  ...
  p => p.ActionTime >= dateonly);
   ...
   p => p.ActionTime <= dateonly);
    ...
    //.Where(M => M.RootObjectTypeId == ObjectTypeALias.RootID).Where(M => M.RootObjectId == AuditTrialAlias.RootObjectId).Where(M => M.AccountId == filter.UserId);
            ...
            var objectTypeAppSite = UnitOfWork.Current.EntitySet<ObjectType>().FirstOrDefault(p => p.ObjectTypeName != null && p.ObjectTypeName== "Noqoush.AdFalcon.Domain.Model.AppSite.AppSite");
            var objectTypeCamp = UnitOfWork.Current.EntitySet<ObjectType>().FirstOrDefault(p => p.ObjectTypeName != null && p.ObjectTypeName == "Noqoush.AdFalcon.Domain.Model.Campaign.Campaign");
            var objectTypeRep = UnitOfWork.Current.EntitySet<ObjectType>().FirstOrDefault(p => p.ObjectTypeName != null && p.ObjectTypeName == "Noqoush.AdFalcon.Domain.Model.Core.ReportScheduler");
            var objectTypeAcc = UnitOfWork.Current.EntitySet<ObjectType>().FirstOrDefault(p => p.ObjectTypeName != null && p.ObjectTypeName == "Noqoush.AdFalcon.Domain.Model.Account.Account");
...
() => ObjectTypeALias.RootID == filter.ObjectTypeID);
            ...
            if (filter.UserId > 0)
            {
                ...
                var ReportSchedulerObjectIdSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Core.ReportScheduler>().Select(M => M.ID)
                ...
                var  AppSiteObjectIdSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.AppSite.AppSite>().Select(M => M.ID)
                    .Where(M => M.Account.ID== filter.UserId).Where(M => M.ID == AuditTrialAlias.RootObjectId);
                ...
                var CampaignObjectIdSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Campaign.Campaign>().Select(M => M.ID)
                   .Where(M => M.Account.ID == filter.UserId).Where(M => M.ID == AuditTrialAlias.RootObjectId);
                ...
                var UserObjectIdSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Account.UserAccounts>().Select(M => M.User.ID)
...
//AccountSubQuery);
  ...
  p => p.ActionTime >= dateonly);
   ...
   p => p.ActionTime <= dateonly);
            ...
            if (!string.IsNullOrEmpty(objectTypeName))
            {
                switch (objectTypeName)
                {
                    case "ReportScheduler":
                        ...
                        ObjectNameSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Core.ReportScheduler>().Select(M => M.ID)
                    ...
                    case "AppSite":
                        ...
                        ObjectNameSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.AppSite.AppSite>().Select(M => M.ID)
                            .Where(M => M.Name.IsInsensitiveLike(Name, MatchMode.Anywhere)).Where(M => M.ID == AuditTrialAlias.RootObjectId);
                        return ObjectNameSubQuery;
                    ...
                    case "Campaign":
                        ...
                        ObjectNameSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Campaign.Campaign>().Select(M => M.ID)
                            .Where(M => M.Name.IsInsensitiveLike(Name, MatchMode.Anywhere)).Where(M => M.ID == AuditTrialAlias.RootObjectId);
                    ...
                    case "Account":
                        ...
                        Noqoush.AdFalcon.Domain.Model.Account.User useralias = null;

                        ObjectNameSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Account.UserAccounts>().Select(M => M.User.ID)
        ...
        {
            ISession nhibernateSession = UnitOfWork.Current.OrmSession<ISession>();
            Noqoush.AdFalcon.Domain.Model.Account.Account accountAlias = null;
            IQueryOver<Noqoush.AdFalcon.Domain.Model.Account.Account, Noqoush.AdFalcon.Domain.Model.Account.Account> rootQuery = nhibernateSession.QueryOver<Noqoush.AdFalcon.Domain.Model.Account.Account>(() => accountAlias);
        ...
        public string GetObjectName(int Id)
        {
            ...
            ISession nhibernateSession = UnitOfWork.Current.OrmSession<ISession>();
            Noqoush.AdFalcon.Domain.Model.Account.User userAlias = null;
            IQueryOver<Noqoush.AdFalcon.Domain.Model.Account.User, Noqoush.AdFalcon.Domain.Model.Account.User> rootQuery = nhibernateSession.QueryOver<Noqoush.AdFalcon.Domain.Model.Account.User>(()=> userAlias);

            //joins
      


            var UseraccountsSubQuery = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Account.UserAccounts>().Select(M => M.Account.ID)
                            .Where(M => M.Account.ID== Id).Where(M => M.User.ID == userAlias.ID);
            var projections = new List<IProjection>();
         
            projections.Add(Projections.Property<Noqoush.AdFalcon.Domain.Model.Account.User>(M => M.FirstName).WithAlias(()=>userAlias.FirstName));
            projections.Add(Projections.Property<Noqoush.AdFalcon.Domain.Model.Account.User>(M => M.LastName).WithAlias(() => userAlias.LastName));
            ...
            rootQuery.TransformUsing(Transformers.AliasToBean<Noqoush.AdFalcon.Domain.Model.Account.User>());
            var users= rootQuery.List< Noqoush.AdFalcon.Domain.Model.Account.User> ();
        ...
        public string GetObjectNameForUserName(int Id)
        {
            ...
            IQueryOver<Noqoush.AdFalcon.Domain.Model.Account.User, Noqoush.AdFalcon.Domain.Model.Account.User> rootQuery = nhibernateSession.QueryOver<Noqoush.AdFalcon.Domain.Model.Account.User>();
...
disjunction);
  ...
  p => p.ActionTime >= dateonly);
   ...
   p => p.ActionTime <= dateonly);
        ...
        public IEnumerable<Noqoush.AdFalcon.Domain.Model.Account.Account> QueryByCratiriaForUsers(Domain.Repositories.Account.UserCriteriaBase criteria, out int Count)
        {
            ...
            Noqoush.AdFalcon.Domain.Model.Account.UserAccounts UserAccountAlias = null;
            ...
            {
                var subqueryPrimaryUser = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.Account.Account>()
    ...
    .Select(gm => gm.ID);
            ...
            if (criteria.publisherUsers)
            {
                ...
                var appSitesAccounts = QueryOver.Of<Noqoush.AdFalcon.Domain.Model.AppSite.AppSite>()
    ...
    .Select(gm => gm.ID);
            ...
            return rootQuery.List<Noqoush.AdFalcon.Domain.Model.Account.Account>();
