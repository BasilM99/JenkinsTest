...
namespace Noqoush.AdFalcon.Domain.Model.Campaign
{
    ...
    public class AdGroup : AdBase<AdGroup, AdGroupStatus>
    {
        ...
        public virtual bool SetAccountFeesSaved(int accountId, int DPPartner = 0)
        {
            ...
            if (costElements != null)
            {
                foreach (var costElm in costElements)
                {
                    ...
                    if (OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().AccountRole == (int)AccountRole.DSP /*|| (OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().ImpersonatedAccount != null && OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().ImpersonatedAccount.AccountRole == (int)AccountRole.DSP)*/)
                    {
        ...
        protected virtual decimal? GetMaxDataBid(AudienceSegmentTargeting AudienceSegmentTargeting)
        {
            ...
            AudicanceBillSummary result = AudienceSegmentTargeting.CalculateBillInternal(Expression.Version_1);
        ...
        public virtual AdGroup Clone(Campaign campaign)
        {
            ...
            if (this.AdGroupInventorySources != null)
            {
                foreach (var invtSrc in GetAdGroupInventorySources())
                {
                    ...
                    invtSrcClone.Campaign = this.Campaign;
        ...
        public virtual void testPublickEventKafka()
        {
            ...
            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new CampaignBillingInfoChangedAck { Id = 134330, RequestedAmount=23,CommittedAmount=20});

            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new AdGroupBillingInfoChangedAck { Id= 157358, RequestedAmount= 23, CommittedAmount =34});

            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new  CampaignOverspend { Id= 134330, Amount=4095});



            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new CampaignBillingInfoChangedAck { Id = 134330, RequestedAmount = 23, CommittedAmount = 20 });

            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new AdGroupBillingInfoChangedAck { Id = 157358, RequestedAmount = 34, CommittedAmount = 34 });



            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new CampaignBillingInfoChangedAck { Id = 134330, RequestedAmount = 3});

            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new AdGroupBillingInfoChangedAck { Id = 157358, RequestedAmount = 34 });

            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new CampaignBillingInfoChangedAck { Id = 134330, CommittedAmount = 20});

            Noqoush.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new AdGroupBillingInfoChangedAck { Id = 157358, CommittedAmount = 34 });
        }
