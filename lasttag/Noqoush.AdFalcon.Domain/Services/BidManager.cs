...
namespace Noqoush.AdFalcon.Domain.Services
{
    ...
    public class BidManager : IBidManager
    {
        ...
        private Noqoush.Framework.ConfigurationSetting.IConfigurationManager configurationManager = null;
        private IList<BidConfig> bidConfigList = null;
        static readonly object lockObj = new object();

        public BidManager(IBidConfigRepository bidConfigRepository, Noqoush.Framework.ConfigurationSetting.IConfigurationManager configurationManager)
        {
        ...
        private IDictionary<int, int> GetDefaultBidValues()
        {
            ...
            var AppScope = Noqoush.AdFalcon.Domain.Common.Model.Core.AppScope.Network;

            if (OperationContext.Current.UserInfo<IUserInfo>().AccountRole == (int)Common.Model.Account.AccountRole.DSP)
            {
                AppScope = Noqoush.AdFalcon.Domain.Common.Model.Core.AppScope.DSP;

            }
            foreach (var item in CostModelWrappers)
            {
                if(AppScope == Noqoush.AdFalcon.Domain.Common.Model.Core.AppScope.DSP)
        ...
        {
            //var AppScope = Noqoush.AdFalcon.Domain.Model.Core.AppScope.Network;

            //if (OperationContext.Current.UserInfo<IUserInfo>().AccountRole == (int)Model.Account.AccountRole.DSP)
            //{
            //    AppScope = Noqoush.AdFalcon.Domain.Model.Core.AppScope.DSP;
