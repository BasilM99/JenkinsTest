...
{
    public class BaseReportGPRepository : RepositoryBase<ChartDto, int> 
    {
        protected Noqoush.Framework.ConfigurationSetting.IConfigurationManager ConfigurationManager = null;
        ...
        public BaseReportGPRepository(RepositoryImplBase<ChartDto, int> repository, Noqoush.Framework.ConfigurationSetting.IConfigurationManager configurationManager)
            : base(repository)
        {
            ConfigurationManager = configurationManager;
        }
   
        protected List<T> GetResult<T>(string script,string optoionalDrop="", string nameofMethod="")
        {
            // Create a PetaPoco database object
            Framework.ApplicationContext.Instance.Logger.Info("star execute script for :" + nameofMethod+" Thread ID:"+ System.Threading.Thread.CurrentThread.ManagedThreadId);
            Framework.ApplicationContext.Instance.Logger.Info(script);
            using (var db = new Noqoush.AdFalcon.Persistence.Reports.RepositoriesGP.PetaPoco.Database("ReportingGPDB"))
            {
                ...
                Framework.ApplicationContext.Instance.Logger.Info("End execute script for :" + nameofMethod + " Thread ID:" + System.Threading.Thread.CurrentThread.ManagedThreadId);
                return results;
            }
