...
{
    public class SummaryGPRepository : RepositoryBase<ChartDto, int>, ISummaryGPRepository
    {
        public SummaryGPRepository(RepositoryImplBase<ChartDto, int> repository)
            : base(repository)
        {
        ...
        public List<EventItemDto> EventItemsList()
        {
            ISession nhibernateSession = UnitOfWork.Current.OrmSession<ISession>();
            ISQLQuery query =
                nhibernateSession.CreateSQLQuery(
                    "SELECT LOWER(Code) as Code, Name FROM account_events UNION SELECT LOWER(Code) as Code, Name FROM ad_events_definition;");
            query.SetTimeout(120);
          
            query.SetResultTransformer(Transformers.AliasToBean<EventItemDto>());
            var items = query.List<EventItemDto>().ToList() ?? new List<EventItemDto>();
