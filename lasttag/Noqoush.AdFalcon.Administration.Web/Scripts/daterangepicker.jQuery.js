...
* --------------------------------------------------------------------
*/
var lastCaleanderActiontaken = 0;
var lastCaleanderActiontakenonElement = null;
jQuery.fn.daterangepicker = function (settings) {
    var rangeInput = jQuery(this);
    
    //Added by Osaleh on 24-01-2012
    jQuery(window).resize(function () {
        if (rp.data('state') == 'open') {
            changeWidth(whatStatus);
            //showRP();
        }
    });


    //Added by Osaleh on 30-09-2011
    //we use this values to localize the date range control
    //---------------------------------- Start ----------------------------------
    var regional = new Array();
    regional['en'] = {
        TodayText: 'Today',
        Last7daysText: 'Last 7 days',
        MonthtodateText: 'Month to date',
        YeartodateText: 'Year to date',
        ThepreviousMonthText: 'The previous Month',
        SpecificDateText: 'Specific Date',
        AllDatesBeforeText: 'All Dates Before',
        AllDatesAfterText: 'All Dates After',
    ...
    regional['ar'] = {
        TodayText: 'اليوم',
        Last7daysText: 'آخر 7 أيام',
        MonthtodateText: 'الشهر حتى الآن',
        YeartodateText: 'السنة حتى تاريخه',
        ThepreviousMonthText: 'الشهر السابق',
        SpecificDateText: 'تاريخ محدد',
        AllDatesBeforeText: 'كل التواريخ قبل',
        AllDatesAfterText: 'كل التواريخ بعد',
        DateRangeText: 'فترة زمنية',
        StartdateText: 'تاريخ البدء',
        EnddateText: 'نهاية التاريخ',
        NextText: 'التالي',
        PrevText: 'السابق',
        DoneText: 'تم',
        ClearText: 'مسح'
    ...
    var options = jQuery.extend({
        presetRanges: [
			{
			    text: regional[currentLang].TodayText, dateStart: 'today', dateEnd: function () {
			        if ($("#DateRecurrenceType").length) {
			            $("#DateRecurrenceType").val(1);
			        }
			        return Date.parse('today');
			    }
			},
			{
			    text: regional[currentLang].Last7daysText, dateStart: 'today-6days', dateEnd: function () {
			        if ($("#DateRecurrenceType").length) {
			            $("#DateRecurrenceType").val(2);
			...
			{
			    text: regional[currentLang].MonthtodateText, dateStart: function () { return Date.parse('today').moveToFirstDayOfMonth(); }, dateEnd: function () {
			        if ($("#DateRecurrenceType").length) {
			            $("#DateRecurrenceType").val(3);
			        }
			        return Date.parse('today');
			    }
			},
			{
			    text: regional[currentLang].YeartodateText, dateStart: function () { var x = Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: function () {
			        if ($("#DateRecurrenceType").length) {
			            $("#DateRecurrenceType").val(4);
			        }
			        return Date.parse('today');
			    }
			},
        //extras:
			{
			    text: regional[currentLang].ThepreviousMonthText, dateStart: function () { return Date.parse('1 month ago').moveToFirstDayOfMonth(); }, dateEnd: function () {

			        if ($("#DateRecurrenceType").length) {
			            $("#DateRecurrenceType").val(5);
			        }
            ...
            { text: regional[currentLang].ClearText, dateStart: '', dateEnd: '' }
        ],
        //presetRanges: array of objects for each menu preset. 
        //Each obj must have text, dateStart, dateEnd. dateStart, dateEnd accept date.js string or a function which returns a date object
        presets: {
            specificDate: regional[currentLang].SpecificDateText,
            allDatesBefore: regional[currentLang].AllDatesBeforeText,
            allDatesAfter: regional[currentLang].AllDatesAfterText,
            dateRange: regional[currentLang].DateRangeText
        },
        rangeStartTitle: regional[currentLang].StartdateText,
        rangeEndTitle: regional[currentLang].EnddateText,
        nextLinkText: regional[currentLang].NextText,
        prevLinkText: regional[currentLang].PrevText,
        doneButtonText: regional[currentLang].DoneText,
        earliestDate: Date.parse("Oct 1, 2011"), //Date.parse('-15years'), //earliest date allowed 
        latestDate: Date.parse('+15years'), //latest date allowed 
        rangeSplitter: '-', //string to use between dates in single input
        dateFormat: 'm/d/yy', // date formatting. Available formats: http://docs.jquery.com/UI/Datepicker/%24.datepicker.formatDate
        closeOnSelect: true, //if a complete selection is made, close the menu
        arrows: false,
        posX: getPosition(rangeInput).left, // x position
    ...
    //Osaleh to remove the animation on ie 7.0
    var timerValue = 400;
    ...
    var datepickerOptions = {
        onSelect: function () {
            ...
            var rangeA_Date = rp.find('.range-start').datepicker('getDate');
            var rangeB_Date = rp.find('.range-end').datepicker('getDate');
            if (rangeA_Date > rangeB_Date) {
                rp.find('.range-end').datepicker("setDate", rangeA_Date);
                rangeB_Date = rangeA_Date;
            }
            var rangeA = fDate(rangeA_Date);
            var rangeB = fDate(rangeB_Date);
    ...
    function showRP() {
        if (rp.data('state') == 'closed') {
            ...
            changeWidth(whatStatus);
            rp.data('state', 'open');
            rp.fadeIn(timerValue);
            options.onOpen();
        }
    }
    function changeWidth(numofCalendars, element) {
        var falg = false;
        
        whatStatus = numofCalendars;

        //if (jQuery.browser.msie) {
        //    if ((jQuery.browser.version.slice(0, 3) == '7.0') && (currentDirection == 'rtl')) {
        //        falg = true;
        //        var width = 235; //jQuery('.ui-widget-content').width();
        //        var cwidth = rpPickers.width();
        //        switch (numofCalendars) {
        //            case 0:
        //                {
        //                    rp.parent().css('left', (rangeInput.position().left - width + 90) + "px");
        //                    break;
        //                }
        //            case 1:
        //                {

        //                    rp.parent().css('left', (rangeInput.position().left - width - 190) + "px");
        //                    break;
        //                }
        //            case 2:
        //                {
        //                    rp.parent().css('left', (rangeInput.position().left - width - 450) + "px");
        //                    break;
        //                }
        //        }
        //    }
        //}
        if (!falg) {
            rp.parent().css(getPositionProperty(), getPosition(rangeInput).left);

            updatePosition();
        }
        //if (numofCalendars>0)
        
        if (((numofCalendars == 2 || numofCalendars == 1) && element )||(lastCaleanderActiontaken > 0 && lastCaleanderActiontakenonElement))
        {
            //
            lastCaleanderActiontaken = numofCalendars;

            if (element) {
                lastCaleanderActiontakenonElement = element.parent().parent().parent();

            }

            if (getPositionRT(lastCaleanderActiontakenonElement).left > 900) {

                //if (numofCalendars == 2 )
                //    rp.parent().css(getPositionProperty(), getPositionRT(rangeInput).left - 290);
                //else
                //    rp.parent().css(getPositionProperty(), getPositionRT(rangeInput).left - 225);
             
                if (lastCaleanderActiontakenonElement.hasClass("ui-daterangepickercontain") && lastCaleanderActiontakenonElement.parent().find(".rangeDateFromC").length > 0) {
                   if (numofCalendars == 2)
                       lastCaleanderActiontakenonElement.css(getPositionProperty(), getPositionRT($(".rangeDateFromC")).left - 290);
                   else {
                       if (getPositionRT(lastCaleanderActiontakenonElement).left + 225 > 1000) {
                           lastCaleanderActiontakenonElement.css(getPositionProperty(), getPositionRT($(".rangeDateFromC")).left - 225);
                   ...
                   if (numofCalendars == 2)
                       lastCaleanderActiontakenonElement.css(getPositionProperty(), getPositionRT(rangeInput).left - 290);
                   else {
                       if (getPositionRT(lastCaleanderActiontakenonElement).left + 225 > 1000) {
                           lastCaleanderActiontakenonElement.css(getPositionProperty(), getPositionRT(rangeInput).left - 225);
                       }
                   }
    ...
    function updatePosition() {
        ...
        rp.parent().css(getPositionProperty(), getPositionRT(rangeInput).left);
        rp.parent().css('top', rangeInput.offset().top + rangeInput.outerHeight());

    }

    function getPositionRT(elemnt) {

        if (currentDirection == 'rtl') {
         
            var elemObj = jQuery(elemnt);
            var offset = elemObj.offset();
            var leftvalue = jQuery(document).width() - offset.left - elemObj[0].offsetWidth;
            return {
                top: offset.top, left: leftvalue
            };
        }
        else {
            return jQuery(elemnt).offset();
        }
    }
    function hideRP() {
        if (rp.data('state') == 'open') {
            rp.data('state', 'closed');
            rp.fadeOut(timerValue);
            options.onClose();
        }
    }
    function toggleRP() {
    ...
    jQuery.fn.clickActions = function (rp, rpPickers, doneBtn) {
        if (jQuery(this).is('.ui-daterangepicker-specificDate')) {
            changeWidth(1, jQuery(this));
            doneBtn.hide();

            rpPickers.show();
            rp.find('.title-start').text(options.presets.specificDate);
            rp.find('.range-start').restoreDateFromData().show(timerValue);
            rp.find('.range-end').restoreDateFromData().hide(timerValue);
            setTimeout(function () { doneBtn.fadeIn(); }, timerValue);
            if ($("#DateRecurrenceType").length) {
                $("#DateRecurrenceType").val(0);
            }
        }
            ...
            rp.find('.range-start').saveDateToData().datepicker('setDate', options.earliestDate).hide(timerValue);
            rp.find('.range-end').restoreDateFromData().show(timerValue);
            setTimeout(function () { doneBtn.fadeIn(); }, timerValue);
            if ($("#DateRecurrenceType").length) {
                $("#DateRecurrenceType").val(0);
            }
            ...
            rp.find('.range-start').restoreDateFromData().show(timerValue);
            rp.find('.range-end').saveDateToData().datepicker('setDate', options.latestDate).hide(timerValue);
            setTimeout(function () { doneBtn.fadeIn(); }, timerValue);
            if ($("#DateRecurrenceType").length) {
                $("#DateRecurrenceType").val(0);
            }
            ...
            changeWidth(0, jQuery(this));
            //custom date range
            doneBtn.hide();

            rp.find('.range-start, .range-end').hide(timerValue, function () {
                rpPickers.hide();
            });
            ...
            if (dateStart != null) {
                ...
                rp.find('.range-start').datepicker('setDate', null).find('.ui-datepicker-current-day').trigger('click');
                rangeInput.eq(0).val("");
                rangeInput.eq(1).val("");
                $(".rangeDateFromC").val("");
               // hideRP();
                ...
                if (rangeInput[0].onchange != null || rangeInput[0].onchange != undefined)
                {
                    rangeInput[0].onchange();
                    return;
                }
                if (rangeInput[1].onchange != null || rangeInput[1].onchange != undefined) {
                    rangeInput[1].onchange();
                    return;
                }

                var customRange=$(".rangeDateFromC");
                if (customRange[0] &&  ( customRange[0].hasOwnProperty("onchange") || customRange[0].onchange != null || customRange[0].onchange != undefined))
                    {
                    customRange[0].onchange();
                        return;
                }
                if (customRange[1] &&  (customRange[1].hasOwnProperty("onchange") || customRange[1].onchange != null || customRange[1].onchange != undefined)) {
                    customRange[1].onchange();
                    return;
                    }
            ...
            if (dateEnd != null) {
                ...
                rp.find('.range-end').datepicker('setDate', null).find('.ui-datepicker-current-day').trigger('click');
                rangeInput.eq(1).val("");
                rangeInput.eq(0).val("");
                $(".rangeDateToC").val("");
               // hideRP();
    ...
    rpPickers.find('.range-start, .range-end').datepicker(options.datepickerOptions); //.datepicker("option", $.datepicker.regional["ar"]);
    rpPickers.find('.range-start').datepicker('setDate', inputDateA); //.datepicker("option", $.datepicker.regional["ar"]); ;
    rpPickers.find('.range-end').datepicker('setDate', inputDateB); //.datepicker("option", $.datepicker.regional["ar"]); ;
    ...
    //    var //clearBtn = jQuery('<button class="btnDone ui-state-default ui-corner-all">Clear</button>')
    //	.click(function () {
    ...
    if (options.posX) {
        rp.parent().css(getPositionProperty(), options.posX);
...
*/
Date.CultureInfo = { name: "en-US", englishName: "English (United States)", nativeName: "English (United States)", dayNames: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], abbreviatedDayNames: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"], shortestDayNames: ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"], firstLetterDayNames: ["S", "M", "T", "W", "T", "F", "S"], monthNames: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"], abbreviatedMonthNames: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], amDesignator: "AM", pmDesignator: "PM", firstDayOfWeek: 0, twoDigitYearMax: 2029, dateElementOrder: "dmy", formatPatterns: { shortDate: "M/d/yyyy", longDate: "dddd, MMMM dd, yyyy", shortTime: "h:mm tt", longTime: "h:mm:ss tt", fullDateTime: "dddd, MMMM dd, yyyy h:mm:ss tt", sortableDateTime: "yyyy-MM-ddTHH:mm:ss", universalSortableDateTime: "yyyy-MM-dd HH:mm:ssZ", rfc1123: "ddd, dd MMM yyyy HH:mm:ss GMT", monthDay: "MMMM dd", yearMonth: "MMMM, yyyy" }, regexPatterns: { jan: /^jan(uary)?/i, feb: /^feb(ruary)?/i, mar: /^mar(ch)?/i, apr: /^apr(il)?/i, may: /^may/i, jun: /^jun(e)?/i, jul: /^jul(y)?/i, aug: /^aug(ust)?/i, sep: /^sep(t(ember)?)?/i, oct: /^oct(ober)?/i, nov: /^nov(ember)?/i, dec: /^dec(ember)?/i, sun: /^su(n(day)?)?/i, mon: /^mo(n(day)?)?/i, tue: /^tu(e(s(day)?)?)?/i, wed: /^we(d(nesday)?)?/i, thu: /^th(u(r(s(day)?)?)?)?/i, fri: /^fr(i(day)?)?/i, sat: /^sa(t(urday)?)?/i, future: /^next/i, past: /^last|past|prev(ious)?/i, add: /^(\+|after|from)/i, subtract: /^(\-|before|ago)/i, yesterday: /^yesterday/i, today: /^t(oday)?/i, tomorrow: /^tomorrow/i, now: /^n(ow)?/i, millisecond: /^ms|milli(second)?s?/i, second: /^sec(ond)?s?/i, minute: /^min(ute)?s?/i, hour: /^h(ou)?rs?/i, week: /^w(ee)?k/i, month: /^m(o(nth)?s?)?/i, day: /^d(ays?)?/i, year: /^y((ea)?rs?)?/i, shortMeridian: /^(a|p)/i, longMeridian: /^(a\.?m?\.?|p\.?m?\.?)/i, timezone: /^((e(s|d)t|c(s|d)t|m(s|d)t|p(s|d)t)|((gmt)?\s*(\+|\-)\s*\d\d\d\d?)|gmt)/i, ordinalSuffix: /^\s*(st|nd|rd|th)/i, timeContext: /^\s*(\:|a|p)/i }, abbreviatedTimeZoneStandard: { GMT: "-000", EST: "-0400", CST: "-0500", MST: "-0600", PST: "-0700" }, abbreviatedTimeZoneDST: { GMT: "-000", EDT: "-0500", CDT: "-0600", MDT: "-0700", PDT: "-0800" } };
