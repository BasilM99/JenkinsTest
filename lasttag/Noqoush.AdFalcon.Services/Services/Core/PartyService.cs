...
namespace Noqoush.AdFalcon.Services.Services.Core
{
    public class PartyService : IPartyService
    {
        ...
        public int GetDemandBusinesPartner()
        {


            return _BusinessPartnerTypeRepository.Query(M => M.Code == "DemandType").SingleOrDefault().ID;

        }

        public int GetSupplyBusinesPartner()
        {


            return _BusinessPartnerTypeRepository.Query(M => M.Code == "SupplyType").SingleOrDefault().ID;

        }

        public int GetDPBusinesPartner()
        {


            return _BusinessPartnerTypeRepository.Query(M => M.Code == "DataProviderType").SingleOrDefault().ID;
        ...
        public PartyListResultDto QueryByCriteria(Noqoush.AdFalcon.Domain.Common.Repositories.Core.PartyCriteria wcriteria)
        {
        ...
        public PartyListResultDto QueryByCriteriaForDPPartner(Noqoush.AdFalcon.Domain.Common.Repositories.Core.DPPartnerCriteria wcriteria)
        {
            DPPartnerCriteria criteria = new DPPartnerCriteria();
            criteria.CopyFromCommonToDomain(wcriteria);
            var result = new PartyListResultDto();
            IEnumerable<DPPartner> list = null;
            list =  _DPPartnerRepository.Query(M => M.IsExternalProvider == true && M.IsDeleted == false && (M.HasAccountWhite == true && M.AccountWhiteList.Any(C => C.Account.ID == Framework.OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().AccountId))).OrderBy(M => M.Name);
        ...
        public IList<PartyDto> GetAllExternalDPPartner(int campId)
        {
            DPPartnerCriteria criteria = new DPPartnerCriteria();

            criteria.Visible = true;
            var camp = _CampaignRepository.Get(campId);
            var result = new PartyListResultDto();
            IEnumerable<DPPartner> list = null;
        ...
        public IList<PartyDto> GetAllInternalDPPartner(int campId)
        {
            DPPartnerCriteria criteria = new DPPartnerCriteria();
        ...
        public PartyDto GetExternalDPPartner(int Id)
        {
            var partItem = _DPPartnerRepository.Query(M => M.ID == Id).SingleOrDefault();
            return MapperHelper.Map<PartyDto>(partItem);
        }
        public EmployeeDto GetEmployee(int id)
        {
            var item = _partyRepository.Get(id);
        ...
        public BusinessPartnerDto GetDPPartnerByAccount(int id)
        {
            var item = _DPPartnerRepository.Query(x => x.Account.ID == id).FirstOrDefault();
        ...
        public bool DeleteParty(int Id)
        {
            if (Id > 0)
            {
                ...
                var ob = this._partyRepository.Get(Id);
                ob.IsDeleted = true;
        ...
        public bool DeleteParties(int[] Ids)
        {
            ...
            if (item is BusinessPartner || item is SSPPartner || item is DSPPartner || item is DPPartner)
            {
                ...
                if (bp_saveDto.AccountList!= null && bp_saveDto.AccountList.Count > 0)
                {
                    ...
                    foreach (var advId in bp_saveDto.AccountList)
                    {
                        ...
                        {
                            bp_item.AccountWhiteList.Add(new BusinessPartnerAccountWhite { Partner = bp_item, Account = new Noqoush.AdFalcon.Domain.Model.Account.Account { ID = advId } });
                        }
                    }
            ...
            where TEntity : Party
        {
            ...
            if ((item is BusinessPartner) && ((item as BusinessPartner).Account!=null) )
            {
                if (item is DSPPartner)
                {
                    ...
                    if (!((item as DSPPartner).Account.PrimaryUser.UserAccounts.Any(M => M.Account.AccountRole == Noqoush.AdFalcon.Domain.Common.Model.Account.AccountRole.DSP)))
                    {
                    ...
                    {
                        var accountobj = (item as DSPPartner).Account.PrimaryUser.UserAccounts.Where(M => M.Account.AccountRole == Noqoush.AdFalcon.Domain.Common.Model.Account.AccountRole.DataProvider).FirstOrDefault();
                ...
                if (item is DPPartner)
                {
                    if (!((item as DPPartner).Account.PrimaryUser.UserAccounts.Any(M => M.Account.AccountRole == Noqoush.AdFalcon.Domain.Common.Model.Account.AccountRole.DataProvider)))
                    {
                    ...
                    {
                        var accountobj = (item as DPPartner).Account.PrimaryUser.UserAccounts.Where(M => M.Account.AccountRole == Noqoush.AdFalcon.Domain.Common.Model.Account.AccountRole.DataProvider).FirstOrDefault();
        ...
        public int SaveEmployee(EmployeeDto saveDto)
        {
            return SaveParty<Employee>(saveDto);
        }

        public int SaveBusinessPartner(BusinessPartnerDto saveDto)
        {
            ...
            if (saveDto.BusinessPartnerTypeId > 0)
            {
                ...
                {
                    return SaveParty<DSPPartner>(saveDto);

                }
                if (type.Code == "SupplyType")
                {
                    return SaveParty<SSPPartner>(saveDto);
                ...
                return SaveParty<BusinessPartner>(saveDto);
            }
            return SaveParty<BusinessPartner>(saveDto);
        }
        ...
        public IList<ImpressionLogDto> GetImpressionLogsNotWrritten(int ProviderId)
        {
            ImpressionLogCriteria criteria = new ImpressionLogCriteria();
            criteria.DataProviderId = ProviderId;
        ...
        public void SaveImpressionLogWrritten(int Id)
        {
           var impItem= _ImpressionLogRepository.Get(Id);
