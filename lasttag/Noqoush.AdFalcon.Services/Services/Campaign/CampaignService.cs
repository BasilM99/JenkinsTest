...
using Noqoush.AdFalcon.Domain.Model.Campaign.Creative;
...
using Noqoush.AdFalcon.Services.Services.Campaign.Creative;
...
namespace Noqoush.AdFalcon.Services.Services.Campaign
{
    public class CampaignService : ICampaignService
    {
        ...
        public CampaignService(IAdGroupEventRepository adGroupEventRepository,  ICampaignFrequencyCappingRepository CampaignFrequencyCappingRepository, ICampaignRepository campaignRepository,
...
IAdvertiserAccountUserRepository AdvertiserAccountReadOnlyUserRepository
        ...
        public int GetAccountAdvertiserId(int advertiserId)
        {

            AdvertiserAccount advertiserAccount = AdvertiserAccountRepository.Query(x => x.Advertiser.ID == advertiserId && x.Account.ID == Framework.OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().AccountId.Value).FirstOrDefault();

            return advertiserAccount.ID;

        }
        public CampaignListResultDto QueryByCratiria(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.CampaignCriteria wcriteria)
        {
            ...
            if (criteria.AdvertiserAccountId.HasValue)
            {
                ...
                AdvertiserAccount advertiserAccount = AdvertiserAccountRepository.Query(x => x.ID == criteria.AdvertiserAccountId.Value && x.Account.ID == Framework.OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().AccountId.Value).FirstOrDefault();
                if (advertiserAccount != null)
                {
            ...
            if (criteria.Page.HasValue)
            {
                ...
                AdvertisorEstimatorCalculation AdvertisorEstimatorCalculationtemp = new AdvertisorEstimatorCalculation(from, to, EstimatorCalculationPeriodType.Accumulated, EstimatorCalculationType.Campaign, Framework.OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().AccountId.Value);
        ...
        public string GetAdvertiserString(int AdvertiserId)
        {

            return AdvertiserRepository.Get(AdvertiserId).Name.ToString();
        }

        public string GetAdvertiserAccountString(int AdvertiserId)
        {

            return AdvertiserAccountRepository.Get(AdvertiserId).Name.ToString();
        }


        public bool IsReadOrWriteCampaign(int CampaignId)
        {
        ...
        public bool IsWriteCampaign(int CampaignId)
        {
            if (OperationContext.Current.UserInfo<AdFalconUserInfo>().IsPrimaryUser)
                return true;
        ...
        public int GetAdvertiserIdFromAccount(int AdvertiserId)
        {

            return AdvertiserAccountRepository.Get(AdvertiserId).Advertiser.ID;
        }
        public bool Delete(IEnumerable<int> campaignIds)
        {
            if (campaignIds != null)
            {
            ...
            if (item != null)
            {
                ...
                if (campaign.CampaignType != CampaignType.AdHouse)
                {
                    ...
                    // && (item.StartDate.CompareTo(Framework.Utilities.Environment.GetServerTime().Date) <= 0))
                    {
                        ...
                        if (item.StartDate != campaign.StartDate)
                        {
                ...
                if (item.EndDate != campaign.EndDate || item.StartDate != campaign.StartDate)
                {
        ...
        public int GetCampaignAdvertiser(int campaignId)
        {
            var item = CampaignRepository.Get(campaignId);
            if (item.Advertiser != null)
            {
                return item.Advertiser.ID;

            }
            return 0;
        }

        public int GetCampaignAdvertiserAccount(int campaignId)
        {
            var item = CampaignRepository.Get(campaignId);
            if (item.AdvertiserAccount != null)
            {
                return item.AdvertiserAccount.ID;

            }
            return 0;
        }
        public CampaignDto Get(int campaignId, CampaignType type, CampaignType Othertype)
        {
            ...
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            if (item.CampaignType != type && item.CampaignType != Othertype)
            {
                throw new DataNotFoundException();
            }
            ValidateCampaign(item);
        ...
        public CampaignSettingsDto GetSettings(int campaignId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public bool SaveSettings(CampaignSettingsDto settingsDto)
        {
        ...
        public bool RemoveDiscount(int campaignId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public IEnumerable<Interfaces.DTOs.Core.TreeDto> GetCampaignsAdvTree(int? AdvertiserId)
        {
            return GetCampaignTreePrep(AdvertiserId);
        }
        ...
        public ResponseDto CloneCampaign(int campaignId, string name)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public string RenameGroup(int campaignId, int GroupId, string name)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
            ...
            if (item.IsValid)
            {
                if (!string.IsNullOrWhiteSpace(name))
                {
                    var group = item.AdGroups.Where(x => x.ID == GroupId).FirstOrDefault();
                    if (group != null)
                    {
                        group.Name = name;
                    }
        ...
        public CampaignServerSettingDto GetServerSettings(int id)
        {
            ...
            if (campaign.IsValid)
            {
                if (campaign.CampaignServerSetting != null)
                {
                    ...
                    if (dto.AgencyCommission == AgencyCommission.FixedCPM)
        ...
        public void SaveServerSetting(CampaignServerSettingDto settingDto)
        {
            ...
            if (item.IsValid)
            {
                ...
                if (settingDto.AgencyCommission == AgencyCommission.FixedCPM)
        ...
        public void DeleteFrequencyCapping(int campaignId, int frequencyCappingId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
            ...
            if (item.IsValid)
            {
                CampaignFrequencyCapping frequencyCappignItem = item.CampaignServerSetting.GetFrequencyCappingList().Where(p => p.ID == frequencyCappingId).SingleOrDefault();
        ...
        public void SaveFrequencyCapping(int campaignId, CampaignFrequencyCappingSaveDto frequencyCappingSave)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
            ...
            if (item.IsValid)
            {
                CampaignFrequencyCapping frequencyCapping = item.CampaignServerSetting.GetFrequencyCappingList().Where(p => p.Event.ID == frequencyCappingSave.EventId).SingleOrDefault();

                if (frequencyCapping != null)
                {
                    frequencyCapping.Interval = frequencyCappingSave.Interval;
                    frequencyCapping.Number = frequencyCappingSave.Number;
                    frequencyCapping.Type = frequencyCappingSave.Type;
                ...
                {
                    frequencyCapping = MapperHelper.Map<CampaignFrequencyCapping>(frequencyCappingSave);
                    frequencyCapping.Event = trackingEventRepository.Get(frequencyCappingSave.EventId);
        ...
        public void SaveCampaignAssignAppsites(CampaignAssignedAppsitesSaveDTo campaignAssignedAppsitesSaveDTo)
        {
            ...
            if (item.IsValid)
            {
                foreach (CampaignAssignedAppsitesDto campaignAssignedAppsitesDto in campaignAssignedAppsitesSaveDTo.InsertedItems)
                {
                    ...
                    campaignAssignedAppsite.AppSite = new Noqoush.AdFalcon.Domain.Model.AppSite.AppSite { ID = campaignAssignedAppsitesDto.Appsite.ID };
                ...
                bool isValid = CheckAppsitesCostModelCompatableWitCampaign(campaignAssignedAppsitesSaveDTo.CampaignId, out notCompatableCampaigns, assignedAppsiteseIds);
        ...
        public CampaignSettingsDto GetCampSettings(int campaignId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public bool SaveCampSettings(CampaignSettingsDto settingsDto)
        {
            var item = CampaignRepository.Get(settingsDto.ID);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public CampaignAssignedAppsitesModelDto GetCampaignAssignAppsites(int campaignId)
        {
        ...
        public CampaignBidConfigModelDto GetCampaignBidConfigs(int campaignId, int adGroupID)
        {
            //var item = CampaignRepository.Get(campaignId);
            //CheckCampaign(item);
            //ValidateCampaign(item);
            Noqoush.AdFalcon.Domain.Model.Campaign.Campaign item = null;
            AdGroup group = null;
            if (adGroupID > 0) {
                group = adGroupRepository.Get(adGroupID);
                if (group != null)
                {
                    item = group.Campaign;
                    ValidateCampaign(item);
                ...
                {
                    item = CampaignRepository.Get(campaignId);
                }
            }
        ...
        public InventorySourceModelDto GetInventorySources(int campaignId, int adGroupID)
        {
            //var item = CampaignRepository.Get(campaignId);
            // CheckCampaign(item);
            // ValidateCampaign(item);
            ...
            // {
            //var group = item.GetGroups().FirstOrDefault(adGroup => adGroup.ID == adGroupID);
            var group = adGroupRepository.Get(adGroupID);
            if (group != null)
            {
        ...
        public IList<PMPDealDto> GetPMPDealConfigConfigs(int campaignId, int adGroupID)
        {
            var item = adGroupRepository.Get(adGroupID);
        ...
        public IList<AdvertiserAccountMasterAppSiteDto> GetMasterListConfigConfigs(int campaignId, int adGroupID)
        {
            var item = adGroupRepository.Get(adGroupID);
        ...
        public AdGroupDealAndSourceDTO GetDealsAndSources(int campaignId, int adGroupID)
        {
        ...
        #region Ad Groups
        public IList<AdGroupDto> GetAllAdGroupByAccount(int AccountId)
        {
            var adGroups = CampaignRepository.GetAllAdGroupByAccount(AccountId);
        ...
        public IEnumerable<Interfaces.DTOs.Core.TreeDto> GetAdGroupsAdvTree(int? AdvertiserId)
        {
            return GetAdGroupAdsPrep(AdvertiserId);
        }
        ...
        public ResponseDto CloneAdGroup(int campaignId, int groupId, string name)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public AdGroupCostElementResultDto GetAdGroupCostElements(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.Creative.AdGroupCostElementCriteria wcriteria)
        {
        ...
        public AdGroupCostElementDto GetAdGroupCostElement(int campaignId, int groupId, int costElemtnId)
        {
        ...
        public AdGroupDynamicBiddingConfigResultDto GetAdGroupDynamicBiddingConfigs(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.Creative.AdGroupCostElementCriteria criteria)
        {
        ...
        public AdGroupDynamicBiddingConfigDto GetAdGroupDynamicBiddingConfig(int campaignId, int groupId, int configId)
        {
            var group = adGroupRepository.Get(groupId);


            if (group != null)
            {

                var element = group.AdGroupDynamicBiddingConfigs.FirstOrDefault(x => x.ID == configId);
        ...
        public AdGroupFeeResultDto GetAdGroupFees(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.Creative.AdGroupCostElementCriteria criteria)
        {
        ...
        public AdGroupFee AddFees(AdGroupFeeDto saveDto, AdGroup group, Noqoush.AdFalcon.Domain.Model.Campaign.Campaign item)
        {
        ...
        public AdGroupFee UpdateFees(AdGroupFeeDto saveDto, AdGroup group, Noqoush.AdFalcon.Domain.Model.Campaign.Campaign item)
        {
        ...
        public void RemoveFees(AdGroupFeeDto saveDto, AdGroup group, Noqoush.AdFalcon.Domain.Model.Campaign.Campaign item)
        {
        ...
        public bool SaveAdRequestTargeting(AdRequestTargetingDto dto)
        {
        ...
        public bool DeleteAdRequestTargeting(int Id)
        {
            if (Id > 0)
            {
                var item = _AdRequestTargetingRepository.Get(Id);
        ...
        public AdRequestTargetingDtoResultDto GetAdRequestTargetings(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.Creative.AdRequestCriteria criteria)
        {
        ...
        public bool SaveImpressionMetricTargeting(ImpressionMetricTargetingDto dto)
        {
            var item = CampaignRepository.Get(dto.campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public bool DeleteImpressionMetricTargeting(int Id)
        {
            if (Id > 0)
            {
                var item = _ImpressionMetricTargetingRepository.Get(Id);
                if (item != null & item.AdGroup.ID > 0)
                {
        ...
        public ImpressionMetricDto GetImpressionMetric(int Id)
        {
            ImpressionMetric ImpressionMetric = _IImpressionMetricRepository.Get(Id);
        ...
        public ImpressionMetricTargetingResultDto GetImpressionMetricTargetings(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.Creative.ImpressionMetricCriteria criteria)
        {
        ...
        public ImpressionMetricTargetingDto GetImpressionMetricTargeting(int TargetingId)
        {
            var result = new ImpressionMetricTargetingResultDto { Items = new List<ImpressionMetricTargetingDto>(), TotalCount = 0 };
            var item = _ImpressionMetricTargetingRepository.Query(x => x.ID == TargetingId).SingleOrDefault();
        ...
        #endregion
        public AdGroupSettingsDto GetAdGroupSettings(int campaignId, int adGroupId)
        {
        ...
        public bool SaveAdGroupSettings(AdGroupSettingsDto settingsDto)
        {
        ...
        public AdGroupSearchDto QueryGroupsByCratiria(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.AdGroupCriteria wcriteria)
        {
            ...
            if (item.IsValid)
            {
                ...
                if (list != null)
                {
                    ...
                    DateTime to = criteria.DateTo == null ? Framework.Utilities.Environment.GetServerDate() : (DateTime)criteria.DateTo;
                    AdvertisorEstimatorCalculation advertisorCalc = new AdvertisorEstimatorCalculation(from, to, EstimatorCalculationPeriodType.Accumulated, EstimatorCalculationType.AdGroup, Framework.OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().AccountId.Value);
        ...
        public bool DeleteGroups(int campaignId, int[] adGroupIds)
        {
            if (adGroupIds != null)
            {
                var item = CampaignRepository.Get(campaignId);
        ...
        public void RunGroups(int campaignId, int[] adGroupIds)
        {
            if (adGroupIds != null)
            {
                var item = CampaignRepository.Get(campaignId);
        ...
        public void PauseGroups(int campaignId, int[] adGroupIds)
        {
            if (adGroupIds != null)
            {
                var item = CampaignRepository.Get(campaignId);
        ...
        public int SaveAdGroup(AdGroupDto adGroup, bool returnId)
        {
            var item = CampaignRepository.Get(adGroup.CampaignId);
            CheckCampaign(item);
            ValidateCampaign(item);

            if (item.IsValid)
            {
                var group = MapperHelper.Map<AdGroup>(adGroup);
                group.Campaign = item;
                ...
                {
                    AdGroup = group,
                    AdAction = adActionTypeRepository.Get((int)adGroup.ActionTypeId),
                    AdType = adGroup.TypeId.HasValue ? adTypeRepository.Get((int)adGroup.TypeId.Value) : null,
                    Objective = adGroupObjectiveTypeRepository.Get((int)adGroup.ObjectiveTypeId),
                };
        ...
        public AdGroupDto GetAdGroup(int campaignId, int adGroupId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public TargetingListDto GetTargeting(int campaignId, int adGroupId)
        {
            // var item = CampaignRepository.Get(campaignId);
            // CheckCampaign(item);
            // ValidateCampaign(item);
            var adGroupObj = adGroupRepository.Get(adGroupId);
            ...
            if (item.IsValid)
            {
                ...
                if (adGroupObj != null)
                {
                    ...
                    var targetings = adGroupObj.Targetings;
                    // get active discount
                    var discount = item.GetActiveDiscount();
                    ...
                    var returnValue = new TargetingListDto
                    {
                        ...
                        CountExternalAudienceList = getCountAudiancesUsedInIntegrationAll(adGroupObj.ID),
                        DataPriceAudienceSegment = getDataBidAudiancesUsedInIntegrationAll(adGroupObj.ID),
                        ...
                        IsHasAds = adGroupObj.GetAds().Any(),
                    ...
                    //returnValue.LoadDefaultsTrackingEvents
                    // if (returnValue.LoadDefaultsTrackingEvents)
                    // {
                    // AddDefaultAdGroupTrackingEvent(adGroupObj, returnValue.CostModelWrapper, null);

                    // returnValue.LoadDefaultsTrackingEvents = false;
                    ...
                    foreach (var iteme in adGroupsTrackings)
                    {
                        ...
                        //var ss = iteme.PreRequisitesList.Select(p => adGroupObj.TrackingEvents.Where(z => z.ID == p).SingleOrDefault());
                        //trackingEvent.PreRequisites = string.Join(",",
                        //                                iteme.PreRequisitesList
                        //                                .Select(p => adGroupObj.TrackingEvents.Where(z => z.ID == p).SingleOrDefault())
                        //                                .Where(p => p != null)
                        //                                .Select(p => p.Code));
                        //trackingEvent.Name = iteme.Description;
                ...
                if (adGroupObj != null)
                {
                  ...
                  /*  else
                    ...
                    if (adGroupObj.BiddingStrategy == BiddingStrategy.Dynamic)
                    {
                        ...
                        //Noqoush.AdFalcon.Domain.Configuration.DynamicBiddingDefaultBidPricePerc

                        adGroupObj.AdGroupDynamicBiddingConfig.MaxBidPrice = targetingSaveDto.MaxBidPrice / adGroupObj.CostModelWrapper.Factor;
                        adGroupObj.AdGroupDynamicBiddingConfig.MinBidPrice = adGroupObj.AdGroupDynamicBiddingConfig.MaxBidPrice * Noqoush.AdFalcon.Domain.Configuration.DynamicBiddingMinBidPricePerc;
                        adGroupObj.AdGroupDynamicBiddingConfig.DefaultBidPrice = adGroupObj.AdGroupDynamicBiddingConfig.MaxBidPrice * Noqoush.AdFalcon.Domain.Configuration.DynamicBiddingDefaultBidPricePerc;

                        adGroupObj.AdGroupDynamicBiddingConfig.BidStep = Noqoush.AdFalcon.Domain.Configuration.DynamicBiddingDefaultBidStep / adGroupObj.CostModelWrapper.Factor;
                    ...
                    if (targetingSaveDto.DeletedTrackingEvents != null && targetingSaveDto.DeletedTrackingEvents.Count != 0)
                    {
                        if (!CheckIfTrackingEventPrerequisiteDeleteIsAllowed(adGroupObj, targetingSaveDto.DeletedTrackingCodeEvents))
                        {
                            throw new BusinessException(new List<ErrorData>() { new ErrorData("DeletePrerequisite") });
                        }

                        foreach (var deletedTrackingEvent in targetingSaveDto.DeletedTrackingCodeEvents)
                        {
                            var deletedTrackingEventObj =
                               adGroupObj.TrackingEvents.FirstOrDefault(targeting => targeting.Code == deletedTrackingEvent && targeting.IsDeleted==false);
                            if (deletedTrackingEventObj != null)
                            {
                                var result = IsDeleteTrackingEventAllowed(targetingSaveDto.CampaignId, targetingSaveDto.AdGroupId, new List<string>() { deletedTrackingEventObj.Code }, true, null);

                                if (result.Key)
                                {
                                    deletedTrackingEventObj.IsDeleted = true;
                                }
                            }
                        }
                    }
                    ...
                    if (targetingSaveDto.InsertedTrackingEvents != null && targetingSaveDto.InsertedTrackingEvents.Count != 0)
                    {
                        foreach (var insertedTrackingEvent in targetingSaveDto.InsertedTrackingEvents)
                        {
                            ...
                            if (CheckEventUniqueByCode(insertedTrackingEvent.Code))
                            {
                   ...
                   // var keywordsAllTargetings = (adGroupObj.Targetings.ToList().OfType<KeywordTargeting>()).ToList();
                    ...
                    IList<CampaignBidConfigDto> notCompatabileAppsites = null;

                    bool isValid = CheckAppsitesCostModelCompatableWitCampaign(targetingSaveDto.CampaignId, out notCompatabileAppsites, targetingSaveDto.InsertedBidConfigItems.Select(x => x.Appsite.ID).ToList(), targetingSaveDto.AdGroupId, targetingSaveDto.CostModelWrapper, isPricingModelChanged);

                    if (notCompatabileAppsites != null && notCompatabileAppsites.Count > 0)
                    {
                        ...
                        foreach (CampaignBidConfigDto bidConfig in notCompatabileAppsites)
                        {
                    ...
                    foreach (var conversionSaveDto in targetingSaveDto.ConversionItems)
                    {
                       ...
                       else if (orginalTracEvent != null)
                        {
                            ...
                            //convEvent.IsConversion = true;
                            //convEvent.IsTracking = true;
                            //convEvent.IsPrimary = conversionSaveDto.IsPrimary;
                            //convEvent.Revenue = conversionSaveDto.Revenue;
                            //convEvent.PixelListsMap = orginalTracEvent.PixelListsMap;
                            //convEvent.AudienceSegmentListsMap = orginalTracEvent.AudienceSegmentListsMap;
                            //adGroupObj.ConversionEvents.Add(convEvent);
                            //CampaignRepository.Save(item);
                            // var orginalEvent3 = adGroupObj.GetConversionEvents().Where(p => p.Code == conversionSaveDto.Code).FirstOrDefault();
        ...
        public string SaveSegmentsForTargeting(int adgroupId, int IdAccAdv, int dpId, List<AudienceSegmentDto> Segments)
        {
            TargetingResultDto dtoResult = new TargetingResultDto();
            string groupAudianceString = string.Empty;
            var adGroupObj = adGroupRepository.Get(adgroupId);
            var ProviderData = _DPPartnerRepository.Get(dpId);
            ...
            //{
            {
                ...
                filter = c => c.Advertiser != null && c.Advertiser.ID == IdAccAdv && c.Provider.ID == dpId && c.IsDeleted == false;
                ...
                {
                    var searchitem = Segments.Where(M => M.OperatorSegmentCode == audItem.OperatorSegmentCode).SingleOrDefault();
                    if (searchitem != null)
                    {
                ...
                {
                    filter = c => c.Parent == null && c.Provider.ID == dpId && c.IsDeleted == false;
                    var maxCode = audianSegRep.GeMaxCode();
                    var parentseg = audianSegRep.Query(filter).FirstOrDefault();
                    Noqoush.AdFalcon.Domain.Model.Campaign.Targeting.group maingrp = new Domain.Model.Campaign.Targeting.group();
                    Noqoush.AdFalcon.Domain.Model.Campaign.Targeting.group grp = new Domain.Model.Campaign.Targeting.group();
                    grp.Operator = "OR";
                    maingrp.Operator = "OR";
                    maingrp.rules = new child[1];
                    // grp.rules[1] = new child { condition="Target"};
                    grp.rules = new child[Segments.Count];
                    for (var i = 0; i < Segments.Count; i++)
                    {
                        Segments[i].Name = new LocalizedStringDto
                        {
                        ...
                        Segments[i].Name.Values.Add(new LocalizedValueDto { ID = 0, Culture = "en-US", Value = ProviderData.Code + ":" + Segments[i].IntegrationId });
                        Segments[i].Name.Values.Add(new LocalizedValueDto { ID = 0, Culture = "ar-JO", Value = ProviderData.Code + ":" + Segments[i].IntegrationId });
                        var segmiteem = MapperHelper.Map<AudienceSegment>(Segments[i]);
                        ...
                        segmiteem.OperatorSegmentCode = ProviderData.Code + ":" + Segments[i].IntegrationId;
        ...
        public string SaveSegmentsForTargetingForDel(int adgroupId, int IdAccAdv, int dpId, List<AudienceSegmentDto> Segments)
        {
            TargetingResultDto dtoResult = new TargetingResultDto();
            string groupAudianceString = string.Empty;
            var adGroupObj = adGroupRepository.Get(adgroupId);
        ...
        public TargetingResultDto AddExternalAudSegmentTargeting(int adgroupId, int IdAccAdv, int dpId, List<AudienceSegmentDto> Segments, string groupAudianceString)
        {
            TargetingResultDto dtoResult = new TargetingResultDto();
            // string groupAudianceString = string.Empty;
            var adGroupObj = adGroupRepository.Get(adgroupId);
            ...
            //{
            {
                ...
                var audienceSegmentTargeting = adGroupObj.Targetings.ToList().OfType<AudienceSegmentTargeting>().Where(M => M.IsExternal == true && M.DataProvider.ID == dpId).FirstOrDefault();
                ...
                if (audienceSegmentTargeting == null)
                {
                    ...
                    string json = string.Empty;
                    if (!string.IsNullOrEmpty(groupAudianceString))
                    {
                        //  json = new JavaScriptSerializer().Serialize(targetingSaveDto.group);

                        // if (targetingSaveDto.changedAudiances == "true")
                        audienceSegmentTargeting.RulesJson = audienceSegmentTargeting.GetRulesJsonForExpression(groupAudianceString);
                        audienceSegmentTargeting.DataProvider = new DPPartner { ID = dpId };
                ...
                else
                {
                    if (!string.IsNullOrEmpty(groupAudianceString))
                    {
                      ...
                      var beforeids=  audienceSegmentTargeting.GetAudienceSegmentIds();
                        ...
                        audienceSegmentTargeting.RulesJson = audienceSegmentTargeting.GetRulesJsonForExpression(groupAudianceString, lastVersion);
                ...
                var audienceSegmentTargetingF = adGroupObj.Targetings.ToList().OfType<AudienceSegmentTargeting>().Where(M => M.IsExternal == true && M.DataProvider.ID == dpId).FirstOrDefault();
                // }
                ...
                if (audienceSegmentTargetingF != null)
                {
                    if (item.Advertiser != null)
                    {
                        var resultsTargetingAudic = audienceSegmentTargetingF.CheckIfRulesHaveAdvertiserBlocker(item.Advertiser.ID, groupAudianceString);
                        if (resultsTargetingAudic)
                        {
                            throw new BusinessException(new List<ErrorData> { new ErrorData("audienceSegmentTargetingNotValid") });
                        }
            ...
            //    }
            //}
            dtoResult.DataPriceAudienceSegment = getDataBidAudiancesUsedInIntegrationAll(adgroupId);
            dtoResult.CountExternalAudienceList = getCountAudiancesUsedInIntegrationAll(adgroupId);
        ...
        public string getAudiancesUsedInIntegration(int adgroupId, int IdAccAdv, int dpId)
        {
            var adGroupObj = adGroupRepository.Get(adgroupId);
        ...
        public string getAudiancesUsedInIntegrationActive(int adgroupId, int IdAccAdv, int dpId)
        {
            var adGroupObj = adGroupRepository.Get(adgroupId);
        ...
        public int getCountAudiancesUsedInIntegration(int adgroupId, int IdAccAdv, int dpId)
        {
            var adGroupObj = adGroupRepository.Get(adgroupId);
            ...
            if (audienceSegmentTargeting != null && !string.IsNullOrEmpty(audienceSegmentTargeting.RulesJson))
            {
                ...
                string jsonObj = audienceSegmentTargeting.GetRulesJsonForGroup(audienceSegmentTargeting.RulesJson);
                return audienceSegmentTargeting.getCountAudienceIntegrationList(jsonObj);
            }
        ...
        public string getDataBidAudiancesUsedInIntegration(int adgroupId, int IdAccAdv, int dpId)
        {
            var adGroupObj = adGroupRepository.Get(adgroupId);
        ...
        public int getCountAudiancesUsedInIntegrationAll(int adgroupId)
        {
            var adGroupObj = adGroupRepository.Get(adgroupId);
        ...
        public string getDataBidAudiancesUsedInIntegrationAll(int adgroupId)
        {
            var adGroupObj = adGroupRepository.Get(adgroupId);
        ...
        public void DeleteTrackingEvent(int campaignId, int adGroupId, int adGroupTrackingEventId)
        {
            var campaign = CampaignRepository.Get(campaignId);
            CheckCampaign(campaign);
            ValidateCampaign(campaign);
            ...
            if (campaign.IsValid)
            {
                ...
                if (adGroup != null && !adGroup.IsDeleted)
                {
                    var trackingEvent = adGroup.TrackingEvents.Where(p => p.ID == adGroupTrackingEventId && !p.IsDeleted).Single();

                    if (adGroup.TrackingEvents.Any(p => !p.IsDeleted && p.PreRequisitesList.Contains(adGroupTrackingEventId)))
                    {
                        throw new BusinessException(new List<ErrorData>() { new ErrorData("DeletePrerequisite") });
                    }
        ...
        public AudicanceBillSummary DeserializeRule(string groupJson)
        {
            ...
            var result = AudienceSegment.CalculateBillInternal(Expression.Version_1);

            return result;
        }

        public IList<AdGroupTrackingEventDto> GetCostModelWrapperTrackingEvents(int campaignId, int adGroupId, int costModelWrapperId)
        {
            //var campaign = CampaignRepository.Get(campaignId);
            //CheckCampaign(campaign);
            //ValidateCampaign(campaign);
        ...
        public bool CheckEventUniqueByCode(string code)
        {
            var accountID = OperationContext.Current.UserInfo<AdFalconUserInfo>().AccountId.Value;
        ...
        {
            return GetAdsTree(OperationContext.Current.UserInfo<AdFalconUserInfo>().AccountId.Value, null);
        }
        public IEnumerable<Interfaces.DTOs.Core.TreeDto> GetAdsAdvTree(int? AdvertiserId)
        {
            return GetAdsAdvTree(OperationContext.Current.UserInfo<AdFalconUserInfo>().AccountId.Value, null, AdvertiserId);
        }
        public IEnumerable<TreeDto> GetAdsTree(int accountId, int? adId = null)
        {
            return GetAdsTreePre(accountId, adId);
        }
        ...
        public int GetMIMEType(string code)
        {
        ...
        public IEnumerable<AdCreativeDtoBase> GetUnApprovedAdsFromAdGroupOfAd(int adId)
        {
            IEnumerable<AdCreativeDtoBase> adCreativeDtoList = new List<AdCreativeDtoBase>();

            var item = adRepository.Get(adId);

            if (item != null && !item.IsDeleted)
            {
                IEnumerable<AdCreative> adsList = item.Group.Ads.Where(p => p.ID != adId && p.Parent == null && !p.IsDeleted && (p.Status.ID != AdCreativeStatus.Active.ID && p.Status.ID != AdCreativeStatus.ActiveAdServer.ID));
        ...
        public AdsSearchDto QueryAdsByCratiria(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.Creative.AdsCriteria wcriteria)
        {
        ...
        public IEnumerable<AdbIDListDto> QueryAdsLessBid(int campaignId, int adGroupId, decimal bid)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
            if (item.IsValid)
            {
                ...
                if (group != null)
                {
                    ...
                    if (groupAds != null)
                    {
                        var list = groupAds.Where(ad => ad.GetReadableBid() < bid && ad.Parent == null);
                        if (list != null)
                        {
        ...
        public IEnumerable<AdbIDListDto> QueryAdsMoreBid(int campaignId, int adGroupId, decimal bid)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public void SetAdsBid(int campaignId, int groupId, int[] adIds, decimal bid)
        {
            if (adIds != null)
            {
                var item = CampaignRepository.Get(campaignId);
                    ...
                    if (group != null)
                    {
                        ...
                        if (groupAds != null)
                        {
                            foreach (var adId in adIds)
                            {
                                ...
                                if (adObj != null)
                                {
                                    item.UpdateAdBid(group, adObj, bid);
                            ...
                            foreach (VideoEndCardCreative videoEndCardCreative in instreamVideoCreative.VideoEndCards)
                            {
                                ...
                                if (videoEndCardCreative.ActionValue != null)
                                {
                                    resultVideoEndCard.ActionValue = new AdActionValueDto
                                    {
                                        ...
                                        Trackers = videoEndCardCreative.ActionValue == null ? null : videoEndCardCreative.ActionValue.Trackers.Where(p => !p.IsDeleted).Select(p => MapperHelper.Map<Noqoush.AdFalcon.Services.Interfaces.DTOs.Campaign.Creative.AdActionValueTrackerDto>(p)).ToList()
                                    };
        ...
        private void getAdCreative(AdCreative item, AdCreativeSaveDto adCreative)
        {
            ...
            if (adCreative.AdActionValue != null)
            {
                ...
                if (adCreative.AdActionValue.Trackers != null)
                {
                    if (adCreative.ID == 0)
                    {
                        item.ActionValue.Trackers = new List<Noqoush.AdFalcon.Domain.Model.Campaign.AdActionValueTracker>();
        ...
        private AdCreativeDto getAdCreative(AdCreative adCreative)
        {
            ...
            var adCreativeDto = new AdCreativeDto
            {
                ...
                AdActionValue = new AdActionValueDto
                {
                    ...
                    Trackers = adCreative.ActionValue == null ? null : adCreative.ActionValue.Trackers.Where(p => !p.IsDeleted).Select(p => MapperHelper.Map<Noqoush.AdFalcon.Services.Interfaces.DTOs.Campaign.Creative.AdActionValueTrackerDto>(p)).ToList()
                },
        ...
        public bool IsAllowedGroupById(int id)
        {
            return CampaignRepository.IsAllowedGroup(id);
        ...
        public bool IsAllowedAdById(int id)
        {
            return CampaignRepository.IsAllowedAd(id);
        }
        ...
        public AdCreativeDto GetAdCreative(int campaignId, int adGroupId, int? adCreativeId, int? adType)
        {
            ...
            if (item.IsValid)
            {
                ...
                if (group != null)
                {
                    ...
                    if (groupAds != null)
                    {
                        if (adCreativeId.HasValue)
                        {
                            var adCreative = groupAds.FirstOrDefault(adItem => adItem.ID == adCreativeId.Value);
                            if (adCreative == null)
                            {
                        ...
                        if (adType.HasValue)
                        {
                            AdType adtype = _AdTypeRepository.Get((int)adType);
        ...
        public IList<VideoMediaFileDto> GetVideoMediaFiles(int videoAdId)
        {
            var mediaFiles = _VideoMediaFileRepository.Query(M => M.AdCreativeUnit.ID == videoAdId).ToList();
        ...
        public int SaveAd(int campaignId, int adGroupId, AdCreativeSaveDto adCreative)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
            ...
            if (item.IsValid)
            {
                ...
                if (group != null)
                {
                    ...
                    var adItem = item.GetGroupAds(group).FirstOrDefault(ad => ad.ID == adCreative.ID);
                    if (adItem != null)
                    {
                        adCreative.IsAdChanged = IsAdChanged(adItem, adCreative);
                    ...
                    {
                        adCreative.AdActionValue = new AdActionValueDto() { Value = group.HouseAd.ForAppSite.GetURL() };
                    }
                    #endregion
                    ...
                    if (adItem == null)
                    {
                        ...
                        switch (adCreative.TypeId)
                        {
                            case AdTypeIds.Text:
                                {
                                    ...
                                    //get Tile Images
                                    if (adCreative.TileImageId > 0)
                                    {
                                        //Not Custom
                                        textAdItem.TileImage = tileImageRepository.Get(adCreative.TileImageId);

                                        foreach (var titlImageDoc in adCreative.TileImageInformationList)
                                        {
                                            var tileImageSize =
                                    ...
                                    getAdCreative(adItem, adCreative);
                            ...
                            case AdTypeIds.Banner:
                                {
                                    ...
                                    {
                                        ActionValue = new Domain.Model.Campaign.Objective.AdActionValue { ActionType = group.Objective.AdAction },
                                        CretiveUnitDeviceType = adCreative.AdBannerType.Value
                                    };
                                    var bannerAdItem = (BannerCreative)adItem;
                                    //get AdCreativeUnits
                                    foreach (var adCreativeUnitDto in adCreative.Banners)
                                    {
                                    ...
                                    getAdCreative(bannerAdItem, adCreative);
                                    adItem.CreationDate = Framework.Utilities.Environment.GetServerTime();
                            ...
                            case AdTypeIds.TrackingAd:
                                {
                                    ...
                                    getAdCreative(adTrackerAdItem, adCreative);


                                    adTrackerAdItem.AppMarketingPartner = appMarketingPartnerRepository.Get(adCreative.AppMarketingPartnerId);
                            ...
                            case AdTypeIds.NativeAd:
                                ...
                                AddNativeAdImages(nativeAdItem, adCreative);
                                AddNativeAdIcons(nativeAdItem, adCreative);
                                getNativeAd(nativeAdItem, adCreative);
                                getAdCreative(nativeAdItem, adCreative);
                                ...
                                var impressionTrackerCreative = adCreative.Banners.Where(p => !string.IsNullOrEmpty(p.ImpressionTrackerRedirect) || !string.IsNullOrEmpty(p.ImpressionTrackerJSRedirect)).SingleOrDefault();
                            ...
                            case AdTypeIds.PlainHTML:
                                {
                                    ...
                                    getAdCreative(plainHtmlCreative, adCreative);
                                    adItem.CreationDate = Framework.Utilities.Environment.GetServerTime();
                                    break;
                                }
                            ...
                            case AdTypeIds.RichMedia:
                                {
                                    adItem = new RichMediaCreative
                                    {
                                        ActionValue = new Domain.Model.Campaign.Objective.AdActionValue { ActionType = group.Objective.AdAction },
                                        CretiveUnitDeviceType = adCreative.AdBannerType.Value,
                                        AdSubType = adCreative.AdSubType,

                                    };
                                    var richMediaCreative = (RichMediaCreative)adItem;
                                    if (adCreative.RichMediaRequiredProtocol.HasValue)
                                    {
                                        richMediaCreative.SetRichMediaProtocol(adCreative.RichMediaRequiredProtocol.Value, adCreative.IsMandatory);  // adCreative.RichMediaRequiredProtocol.HasValue ? requiredProtocolRepository.Get(adCreative.RichMediaRequiredProtocol.Value).Name : null;
                                    }
                                    richMediaCreative.ClickMethod = adCreative.ClickMethod;
                                    //get Ad Creative Unit
                                    foreach (var adCreativeUnitDto in adCreative.Banners)
                                    {
                                    ...
                                    if (adCreative.ClickTags != null && adCreative.ClickTags.Count > 0)
                                        SetClickTags(richMediaCreative, adCreative.ClickTags.ToList());
                                    getAdCreative(richMediaCreative, adCreative);
                                    adItem.CreationDate = Framework.Utilities.Environment.GetServerTime();
                                    break;
                                }
                            case AdTypeIds.InStreamVideo:
                                {
                                    ...
                                    InStreamVideoCreative.VideoEndCardFluid = adCreative.VideoEndCardFluid;
                                    //   AddMediaFilesForInstream(InStreamVideoCreative, adCreative, adCreative.InStreamVideos[0].InStreamVideoCreativeUnit.VideoType, adCreative.InStreamVideos[0].InStreamVideoCreativeUnit.DeliveryMethod);

                                    foreach (AdCreativeUnitDto adCreativeUnitDto in adCreative.InStreamVideos)
                                    {
                                        ...
                                        if (!adCreativeUnitDto.InStreamVideoCreativeUnit.IsVideo)
                                        {
                                            ...
                                            InStreamVideoCreative.OrientationType = adCreative.OrientationType != OrientationType.Any ? adCreative.OrientationType : (OrientationType?)null;

                                            InStreamVideoCreative.DurationInSeconds = adCreative.VideoMediaFile.duration;
                                            InStreamVideoCreative.Description = adCreative.Description;
                                            inStreamAdCreativeUnit.BitRate = adCreative.VideoMediaFile.bitRate;
                                            inStreamAdCreativeUnit.VideoType = mimeTypeRepository.Get(adCreativeUnitDto.InStreamVideoCreativeUnit.VideoType);
                                            inStreamAdCreativeUnit.Width = adCreative.VideoMediaFile.width;
                                            inStreamAdCreativeUnit.Height = adCreative.VideoMediaFile.height;
                                            ...
                                            AddMediaFilesForInstream(InStreamVideoCreative, adCreative, adCreative.InStreamVideos[0].InStreamVideoCreativeUnit.DeliveryMethod, inStreamAdCreativeUnit.AdCreativeUnit);

                                            if (adCreative.VASTProtocol == VASTProtocolsVersion.VAST2)
                                                inStreamAdCreativeUnit.AdCreativeUnit.Protocol = _ProtocolRepository.GetByCode((int)ProtocolCode.VAST_2);
                                            else if (adCreative.VASTProtocol == VASTProtocolsVersion.VAST3)
                                                inStreamAdCreativeUnit.AdCreativeUnit.Protocol = _ProtocolRepository.GetByCode((int)ProtocolCode.VAST_3);
                                            else if (adCreative.VASTProtocol == VASTProtocolsVersion.VAST4)
                                                inStreamAdCreativeUnit.AdCreativeUnit.Protocol = _ProtocolRepository.GetByCode((int)ProtocolCode.VAST_4);
                                            else if (adCreative.VASTProtocol == VASTProtocolsVersion.VAST41)
                                                inStreamAdCreativeUnit.AdCreativeUnit.Protocol = _ProtocolRepository.GetByCode((int)ProtocolCode.VAST41);
                                            else if (adCreative.VASTProtocol == VASTProtocolsVersion.VAST42)
                                        ...
                                        if (adCreative.ImpressionTrackingURL != null && adCreative.ImpressionTrackingURL.Count > 0)
                                        {
                                            // foreach (var URL in adCreative.VideoEndCardsTrackingURL)
                                            ////  inStreamAdCreativeUnit.AdCreativeUnit
                                            inStreamAdCreativeUnit.AdCreativeUnit.SetTrackingEvent(impressionEventObj, adCreative.ImpressionTrackingURL);
                                            //}
                                        }
                                    }
                                    ...
                                    if (adCreative.VideoEndCards != null && adCreative.VideoEndCards.Count > 0)
                                    {
                                        var impressionEventobj = group.TrackingEvents.Where(p => p.Code.ToLower() == IMPRESSIONEVENT && !p.IsDeleted).FirstOrDefault();
                                        foreach (var adCreativeSave in adCreative.VideoEndCards)
                                        {
                                            if (InStreamVideoCreative.VideoEndCards == null)
                                            ...
                                            foreach (var adCreativeUnitDto in adCreativeSave.CreativeUnitsContent)
                                            {
                                                ...
                                                if (adCreative.VideoEndCardsTrackingURL != null && adCreative.VideoEndCardsTrackingURL.Count > 0)
                                                {
                                                    //foreach (var URL in adCreative.VideoEndCardsTrackingURL)
                                                    //{
                                                    adCreativeUnit.SetTrackingEvent(impressionEventobj, adCreative.VideoEndCardsTrackingURL);
                                                    //}
                                                }
                                            ...
                                            item.SetCreativeStatus(group, adVideoEndCardItem, adCreative.IsAdChanged);
                                    ...
                                    if (adCreative.ThirdPartyTrackers != null && adCreative.ThirdPartyTrackers.Count > 0)
                                        SetThirdPartyTrackers(adItem, adCreative.ThirdPartyTrackers.ToList());
                                    break;
                                }
                        ...
                        if (adItem != null)
                        {
                            ...
                            adItem.EnableEventsPostback = adCreative.EnableEventsPostback;
                            adItem.VerifyTargetingCriteria = adCreative.VerifyTargetingCriteria;

                            adItem.VerifyPrerequisiteEvents = adCreative.VerifyPrerequisiteEvents;
                            adItem.VerifyDailyBudget = adCreative.VerifyDailyBudget;
                            adItem.VerifyCampaignStartAndEndDate = adCreative.VerifyCampaignStartAndEndDate;
                            adItem.ValidateRequestDeviceAndLocationData = adCreative.ValidateRequestDeviceAndLocationData;

                            adItem.UpdateEventsFrequency = adCreative.UpdateEventsFrequency;


                            adItem.UpdateTags = adCreative.UpdateTags;
                            adItem.VerifyEventsFrequency = adCreative.VerifyEventsFrequency;
                            ...
                            case AdTypeIds.Text:
                                {
                                    ...
                                    if ((textAdItem.TileImage == null) || (adCreative.TileImageId < 0) || (textAdItem.TileImage.IsCustom))
                                    {
                                        //Custom Tile Image
                            ...
                            case AdTypeIds.Banner:
                                {
                                    ...
                                    foreach (var adCreativeUnit in from creativeUnit in adCreativeUnits
                                                                   where (adCreative.Banners.Any(x => x.CreativeUnitId == creativeUnit.CreativeUnit.ID) == false)
                                                                   select creativeUnit)
                                    {
                            ...
                            case AdTypeIds.TrackingAd:
                                {
                                    ...
                                    //    }
                                    //}
                                    getAdCreative(addTrackinAdItem, adCreative);

                                    addTrackinAdItem.AppMarketingPartner = appMarketingPartnerRepository.Get(adCreative.AppMarketingPartnerId);
                                    //To do we need to formate
                                    //adTrackerAdItem.ClickTrackerUrl = adCreative.ClickTrackerUrl;
                            ...
                            case AdTypeIds.NativeAd:
                                {
                                    ...
                                    var impressionTrackerCreative = adCreative.Banners.Where(p => !string.IsNullOrEmpty(p.ImpressionTrackerRedirect)).SingleOrDefault();
                            ...
                            case AdTypeIds.RichMedia:
                                {
                                    ...
                                    if (adCreative.RichMediaRequiredProtocol.HasValue)
                                    {
                                        richMediaCreative.SetRichMediaProtocol(adCreative.RichMediaRequiredProtocol.Value, adCreative.IsMandatory);
                                        //  richMediaCreative.RichMediaRequiredProtocol =;// : RichMediaProtocols.MRAID1; ;//requiredProtocolRepository.Get(adCreative.RichMediaRequiredProtocol.Value) : null;
                                    }
                            ...
                            case AdTypeIds.InStreamVideo:
                                {
                                    ...
                                    getAdCreative(inStreamVideoCreative, adCreative);
                                    inStreamVideoCreative.Description = adCreative.Description;
                                    inStreamVideoCreative.VideoEndCardFluid = adCreative.VideoEndCardFluid;
                                    foreach (AdCreativeUnitDto adCreativeUnitDto in adCreative.InStreamVideos)
                                    {
                                        ...
                                        if (!adCreativeUnitDto.InStreamVideoCreativeUnit.IsVideo)
                                        {
                                            ...
                                            else if (!string.IsNullOrEmpty(adCreativeUnitDto.InStreamVideoCreativeUnit.Xml) && !adCreativeUnitDto.InStreamVideoCreativeUnit.IsXmlUrl)
                                            {
                                                ...
                                                xmldoc = new Document
                                                {
                                                    Name = adCreative.Name + adCreativeUnit.Version.ToString(),// Framework.Utilities.Environment.GetServerTime().Replace('/','_'),
                                                    Content = Xml,
                                                    Size = Xml.Length,
                                                    DocumentType = documentTpye,
                                                    UploadedDate = Framework.Utilities.Environment.GetServerTime(),
                                            ...
                                            inStreamVideoCreative.DurationInSeconds = adCreative.VideoMediaFile.duration;
                                            adCreativeUnit.InStreamVideoCreativeUnit.BitRate = adCreative.VideoMediaFile.bitRate;
                                            ...
                                            adCreativeUnit.InStreamVideoCreativeUnit.Width = adCreative.VideoMediaFile.width;
                                            adCreativeUnit.InStreamVideoCreativeUnit.Height = adCreative.VideoMediaFile.height;
                                            AddMediaFilesForInstream(inStreamVideoCreative, adCreative, adCreative.InStreamVideos[0].InStreamVideoCreativeUnit.DeliveryMethod, adCreativeUnit);
                                        ...
                                        if (adCreative.ImpressionTrackingURL != null && adCreative.ImpressionTrackingURL.Count > 0)
                                        {
                                            // foreach (var URL in adCreative.VideoEndCardsTrackingURL)
                                            //// {
                                            adCreativeUnit.SetTrackingEvent(impressionEventObj, adCreative.ImpressionTrackingURL);
                                            //}
                                        }
                                    }
                                    ...
                                    if (adCreative.VideoEndCards != null && adCreative.VideoEndCards.Count > 0)
                                    {
                                        ...
                                        if (inStreamVideoCreative.VideoEndCards != null)
                                        {
                                            ...
                                            {
                                                if (adCreative.VideoEndCards != null)
                                                    itemVideoCard = adCreative.VideoEndCards.Where(M => M.CreativeUnitsContent != null && M.CreativeUnitsContent.Any(X => X.CreativeUnitId == inStreamVideoCreative.VideoEndCards[i].AdCreativeUnits[0].CreativeUnit.ID)).SingleOrDefault();
                    ...
                    {
                        adItem.SetAdCreativeBid(adCreative.Bid, adItem.Group.CostModelWrapper.Factor);
                    }
                    adItem.Validate(statusCheck: true);
                    item.SetCreativeStatus(group, adItem, adCreative.IsAdChanged);
                    ...
                    adItem.IsSecureCompliant = SearchForSecuredUrls(adItem, adCreative);
                    adItem.Type = _AdTypeRepository.Get((int)adItem.TypeId);
                    if (adItem.ID > 0 && adCreative.IsAdChanged)
                    ...
                    //if (adCreative.CreativeVendorIds == null || adCreative.CreativeVendorIds.Count==0)
                    //{

                    ////Framework.Caching.CacheManager.Current.DefaultProvider.Put("", listOfKywordsDto.ToArray());

                    //}
                    if (adCreative.IsCreativeVendorChanged)
                    {
                        foreach (var creativeUnit in adItem.AdCreativeUnits)
                        {

                            SaveUpdateAdCreativeUnit(creativeUnit, adCreative.CreativeVendorIds);
                        }
                    }
                    else
                    {
                        if ((adCreative.CreativeVendorIds == null || adCreative.CreativeVendorIds.Count() == 0) || (adCreative.IsAdChanged && !adCreative.IsCreativeVendorChanged))
                        {
                            ...
                            foreach (var creativeUnit in adItem.AdCreativeUnits)
                            {
                                ...
                                listOfKeyWords = Noqoush.Framework.Caching.RedisCache.RedisCacheSerilizer.FormatterSurrogateDeSerialize(Framework.Caching.CacheManager.Current.DefaultProvider.Get<byte[]>("CreativeVendorList")) as IEnumerable<CreativeVendorKeywordDto>;
                                ...
                                if (listOfKeyWords == null || listOfKeyWords.Count() == 0)
                                {
                                    ...
                                    if (listOfKeyWords != null)
                                        Framework.Caching.CacheManager.Current.DefaultProvider.Put<byte[]>("CreativeVendorList", Noqoush.Framework.Caching.RedisCache.RedisCacheSerilizer.FormatterSurrogateSerialize(listOfKeyWords.ToList()));
                                }
                                ...
                                if (adCreative.AdActionValue != null && adCreative.AdActionValue.Trackers != null)
                                {
                                    foreach (AdActionValueTrackerDto Tracker in adCreative.AdActionValue.Trackers)
                                    {
                                        // UrlsListToCheck.Add(Tracker.TrackingUrl);
                            ...
                            if (adCreative.CreativeVendorIds != null)
                                creativeVendorIds = creativeVendorIds.Concat(adCreative.CreativeVendorIds).Distinct().ToList();
                            foreach (var creativeUnit in adItem.AdCreativeUnits)
                            {
                    ...
                    if (!IsCampaignManager() &&  !SearchForAllowImpressionTrackers(group, adItem, adCreative))
                    {
                    ...
                    adItem.SetWrapperContent(adCreative.WrapperContent);
                    #endregion
                    CampaignRepository.Save(item);
        ...
        public bool DoesContainDataProviderAllowImpressionTracker(int campaignId, int groupId)
        {
        ...
        public void GenerateClickTrackerUrl(AdCreative aditem, string uniqID)
        {
            ...
            if (constraints != null)
            {
                ...
                if (constraint != null)
                {
                    ...
                    var platformID = constraint.Platform.ID;
                    var appMarketingPartnerObj = adTrackerCreative.AppMarketingPartner;
                    var trakers = appMarketingPartnerObj.Trackers.Where(M => M.Platform.ID == platformID && M.TypeID == 1).ToList();
        ...
        public bool DeleteAds(int campaignId, int groupId, int[] adIds)
        {
            if (adIds != null)
            {
                var item = CampaignRepository.Get(campaignId);
        ...
        /// <param name="groupId">Group Id to Get By</param>
        /// <param name="adIds">Ids to Get By</param>
        public void RunAds(int campaignId, int groupId, int[] adIds)
        {
            if (adIds != null)
            {
                var item = CampaignRepository.Get(campaignId);
        ...
        /// <param name="groupId">Group Id to Get By</param>
        /// <param name="adIds">Ids to Get By</param>
        public void PauseAds(int campaignId, int groupId, int[] adIds)
        {
            if (adIds != null)
            {
                var item = CampaignRepository.Get(campaignId);
        ...
        public void RejectAd(int campaignId, int groupId, int adId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
                    ...
                    if (groupAds != null)
                    {
                        ...
                        if (adObj != null)
                        {
                            ...
                            foreach (var adCreative in adObj.AdCreativeUnits)
                            {
                                ...
                                var creativeAttributes = approveAdDto.AdCreativesAttribues.Where(p => p.ID == adCreative.ID).SingleOrDefault();
                        ...
                        if (item.Account.AccountRole == AccountRole.DSP)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (var sspPartnter in sspPartnters)
                                {
                                    if (approveAdDto.DeletedAppSiteIds.Where(x => x == sspPartnter.AppSite.ID).Count() > 0)
                                    {
                        ...
                        else
                        {
                            if (approveAdDto.AppSiteIds != null)
                            {
                                ...
                                IList<CampaignBidConfigDto> notCompatabileAppsites = null;
                                CheckAppsitesCostModelCompatableWitCampaign(item.ID, out notCompatabileAppsites, newAppsiteIds, adObj.Group.ID);
                                if (notCompatabileAppsites != null && notCompatabileAppsites.Count > 0)
                                {
                                ...
                                {
                                    Noqoush.AdFalcon.Domain.Model.AppSite.AppSite appSite = new Noqoush.AdFalcon.Domain.Model.AppSite.AppSite { ID = appSiteId };
                                    if (appSite != null)
                                    {
        ...
        /// </summary>
        /// <returns></returns>
        public AdCreativeSummaryDto GetAdSummary(int campaignId, int groupId, int adId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ...
            if (item.IsValid)
            {
                ...
                if (group != null)
                {
                    var adItem = item.GetGroupAds(group).FirstOrDefault(ad => ad.ID == adId);
        ...
        public AdCreativeFullSummaryDto GetAdFullSummary(int campaignId, int groupId, int adId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public IList<CampaignSummaryDto> GetAdsSummary(Noqoush.AdFalcon.Domain.Common.Repositories.Campaign.Creative.AdsSummaryCriteria wcriteria)
        {
        ...
        public FormattedContentDto FormatAdCreativeContent(string content, int creativeId = 0)
        {
            ...
            if (creativeId > 0)
                adCreativeUnit.CreativeUnit = _CreativeUnitRepository.Get(creativeId);
        ...
        public ResponseDto CloneAd(int campaignId, int groupId, int adId, string name)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public bool IsFormattedAdCreativeContent(string content)
        {
            var adCreativeUnit = new AdCreativeUnit() { Content = content };
            var adcreativeFormatter = AdCreativeContentFormatterBase.GetAdCreativeContentFormatter(adCreativeUnit);
            return adcreativeFormatter.IsFormatted();
        }
        ...
        public void AddDefaultAdGroupTrackingEventById(int adGroupId, int costModelWrapperId, int? oldCostModelWrapper)
        {

        var adgroupObj=     adGroupRepository.Get(adGroupId);
            adgroupObj.CostModelWrapper = new CostModelWrapper { ID= costModelWrapperId };
            AddDefaultAdGroupTrackingEvent(adgroupObj, costModelWrapperId, oldCostModelWrapper);
        }
        ...
        private void updateAccountTrackingEvents(AdGroup adGroup, AdGroupTrackingEvent adGroupTrackingEvent)
        {
            ...
            Noqoush.AdFalcon.Domain.Model.Account.AccountTrackingEvents AccountTrackingEvent = null;
            //already exists
            if (Event != null)
            {
                Event.Description = adGroupTrackingEvent.Description;
            ...
            {
                AccountTrackingEvent = MapperHelper.Map<Noqoush.AdFalcon.Domain.Model.Account.AccountTrackingEvents>(adGroupTrackingEvent);
        ...
        public bool checkSystemEventFraud(string code, string name)
        {
            var SystemTrackingEvent = trackingEventRepository.GetAll().ToList();
            var result = SystemTrackingEvent.Where(x => x.Code == code || x.GetDescription() == name).FirstOrDefault();
            return result != null;
        }
        ...
        /// <returns></returns>
        public bool CheckAppsitesCostModelCompatableWitCampaign(int campaignId, out IList<CampaignBidConfigDto> notCompatibleAppSiteList, List<int> appsites = null, int? goupId = null, int? groupCostModelWrapperID = null, bool checkExisting = false)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
            ...
            if (item.IsValid)
            {
                ...
                foreach (int adGoupId in adGroupIds)
                {
                    ...
                    if (group != null)
                    {
                        if (!groupCostModelWrapperID.HasValue)
                        {
                            if (group.CostModelWrapper == null)
                                continue;

                            groupCostModelWrapperID = group.CostModelWrapper.CostModel.ID;
                        }
                        ...
                        Noqoush.AdFalcon.Domain.Model.AppSite.AppSite appsite = null;
                        ...
                        foreach (int appSiteId in allAppsites.Distinct())
                        {
                            ...
                            if (appsitePricingModel != null && appsitePricingModel.ID != groupCostModelWrapperID)
                            {
        ...
        public List<int> GetAppSiteAdQueues(int campaignId, int adGoupId, int adId)
        {
            var item = CampaignRepository.Get(campaignId);
            CheckCampaign(item);
            ValidateCampaign(item);
        ...
        public AdTypeIds GetAddTypesByAddGroupAction(int adGroupId)
        {

            var Objective = this.adGroupRepository.Query(item => item.ID == adGroupId).Select(M => M.Objective).SingleOrDefault();

            if (Objective.AdAction.ID >= 2 && Objective.AdAction.ID <= 5)
            {
                return AdTypeIds.TrackingAd;

            }
            return AdTypeIds.Undefined;
        }
        public int GetActionTypeByadGroup(int adGroupId)
        {

            var Objective = this.adGroupRepository.Query(item => item.ID == adGroupId).Select(M => M.Objective).SingleOrDefault();

            return Objective.AdAction.ID;
        }
        public int GetCreativeVendorForAdCreativeUnit(int AdCreativeUnitId)
        {

            var adCreativeUnitVendor = _AdCreativeUnitVendorRepository.Query(M => M.Unit.ID == AdCreativeUnitId).FirstOrDefault();

            if (adCreativeUnitVendor != null)
                return adCreativeUnitVendor.Vendor.ID;
        ...
        public void PublishVideoAd(int videoAdint)
        {

            var videoAd = _InStreamVideoCreativeRepository.Get(videoAdint);
        ...
        public void UploadTest(string fullDirectory, byte[] Content)
        {
            Noqoush.Framework.Utilities.FtpHelper.Upload(fullDirectory, Content);
        ...
        public string GetDirectory(int index)
        {
            try
            {
                ...
                subFolder = Noqoush.Framework.Utilities.Environment.GetServerTime().ToString("yyyyMMdd");
                temp = string.Format("{0}/{1}", baseDirectory, subFolder);
                //create folder fo the current date
                Noqoush.Framework.Utilities.FtpHelper.CreateDirectory(temp);
                //create folder for current Campaign
                var isFolderCreated = false;
                while (!isFolderCreated)
                {
                    temp = string.Format("{0}/{1}", baseDirectory, subFolder);
                    var r = (index * RandomNumber(1, 100000)).ToString();
                    temp = string.Format("{0}/{1}", temp, r);
                    if (!Noqoush.Framework.Utilities.FtpHelper.DirectoryExists(temp))
                    {
                        Noqoush.Framework.Utilities.FtpHelper.CreateDirectory(temp);
                ...
                subFolder = string.Format("{0}/{1}", subFolder, (index * RandomNumber(1, 100000)).ToString());
                temp = string.Format("{0}/{1}", baseDirectory, subFolder);
        ...
        #region advertiser
        public int GetAdvertiserIdByCampaignId(int id)
        {
            var camp = CampaignRepository.Get(id);
            return camp.Advertiser != null ? camp.Advertiser.ID : 0;

        }
        public int GetAdvertiserAccountIdByCampaignId(int id)
        {
            var camp = CampaignRepository.Get(id);
            return camp.AdvertiserAccount != null ? camp.AdvertiserAccount.ID : 0;
        ...
        public bool IsllowedAdvertiserForCamp(Noqoush.AdFalcon.Domain.Model.Campaign.Campaign camp)
        {
            if (OperationContext.Current.UserInfo<AdFalconUserInfo>().IsPrimaryUser)
                return true;
            ...
            var obj = _advertiserAccountUserRepository.Query(x => x.Link.ID == advLink.ID && x.IsDeleted == false && x.User.ID == Framework.OperationContext.Current.UserInfo<Noqoush.Framework.UserInfo.IUserInfo>().UserId.Value).FirstOrDefault();
            if (obj != null)
                return obj.Write;
        ...
        public int GetPublisherCounterCurrentWeek()
        {
            var nhibernateSession = UnitOfWork.Current.OrmSession<ISession>();
            ...
            return Convert.ToInt32(DateTimeObj.ToString("yyyyMMdd"));
        }

        public int GetAudienceListCounter(string dpName)
        {
            var nhibernateSession = UnitOfWork.Current.OrmSession<ISession>();
        ...
        public bool IsFormatedAdCreativUnit(string content)
        {
            var adCreativeUnit = new AdCreativeUnit() { Content = content };
            var adcreativeFormatter = AdCreativeContentFormatterBase.GetAdCreativeContentFormatter(adCreativeUnit);
            return adcreativeFormatter.IsFormatted();
