...
class AppGrid extends BaseFormComponent {
    ...
    renderArrowMenu = (obj, value) => {
        ...
        this.props.gridConfigItem.arrowMenu.forEach((element) => {
            ...
            if (tempAttributes.hasOwnProperty("attributes")) {
                ...
                if (tempAttributes.attributes.hasOwnProperty("href"))
                    tempAttributes.attributes.href = element.attributes.href.replace(
                        "{{id}}",
                        eval("obj." + this.props.gridConfigItem.colDefObj.itemIds)
                    );
                ...
                if (tempAttributes.attributes.hasOwnProperty("item_extra_info"))
                    ...
                    if (tempAttributes.attributes.hasOwnProperty("callbackfunction"))
                        {
                          temCallback = tempAttributes.attributes.callbackfunction;
                          //tempAttributes.attributes.callbackfunction = eval('tempAttributes.attributes.callbackfunction')
                        }
            }
    ...
    getCells = (row, _isThereSubGrids) => {
        ...
        for (
            ...
            var cellValue = null;
            // if(this.props.gridConfigItem.colDef.length > 0 && this.props.gridConfigItem.colDef[0] == ""){
            //   //contentType
            if (typeof (this.props.gridConfigItem.headers[index + 1]) != "undefined" &&
                this.props.gridConfigItem.headers[index + 1].hasOwnProperty("contentType")
                && this.props.gridConfigItem.headers[index + 1].contentType == "link") {
                cellValue = <a href={this.PrepareLinkHref(this.props.gridConfigItem.headers[index + 1].linkContent, row)}>{row[element]}</a>;
            } else {
                cellValue = row[element];
            }
            // }
            // else if(this.props.gridConfigItem.colDef.length > 0 && this.props.gridConfigItem.colDef[0] != ""){
            //   if(typeof(this.props.gridConfigItem.headers[index + 2]) != "undefined" &&
            //   this.props.gridConfigItem.headers[index + 2].hasOwnProperty("contentType")
            // && this.props.gridConfigItem.headers[index + 2].contentType == "link")
            // {
            //   cellValue = <a href={this.props.gridConfigItem.headers[index + 2].linkContent + row["Id"]}>{row[element] }</a>;
            // }else{
            //   cellValue = row[element] ;
            // }
            //}
    ...
    handelPageRefresh = (flag) => {
        ...
        const { paging } = this.props;
        if (this.props.gridConfigItem.gridDataFilter && !(Object.keys(this.props.gridConfigItem.gridDataFilter).length === 0 && this.props.gridConfigItem.gridDataFilter.constructor === Object)) {
    ...
    PrepareLinkHref = (linkText, row) => {
        //debugger;
        let Params = this.props.gridConfigItem.Params;
        let i = 0;
        while (linkText.includes("val") && Params && i < Object.keys(Params).length) {
            if (Params["val" + i]) {
                linkText = linkText.replace("{{val" + i + "}}", row[Params["val" + i]]);
            }
            i++;
        }
        i = 0;
