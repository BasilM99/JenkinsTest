...
class AppGrid extends BaseFormComponent {
  ...
  get PagingDisplayItems() {
    ...
    return (
      <span>
        {this.TFormat("paging.dispalyingItems", [
          from,
          parseInt(to),
          paging.countItems,
        ])}
          ...
          <tbody>
            {this.props.data != null && this.props.data.length > 0 ? (
              this.props.data.map((row, index) => {
                ...
                this.props.headers != null && this.props.headers.length > 0
              ) ? (
              <React.Fragment></React.Fragment>
            ) : (
              <tr>
                <td colSpan="1333">{T("msg.noRecords")}</td>
              </tr>
            )}
          </tbody>
        </table>
  ...
  handelPageRefresh = (flag) => {
    ...
    if (
      !(
        Object.keys(this.props.gridConfigItem.gridDataFilter).length === 0 &&
        this.props.gridConfigItem.gridDataFilter.constructor === Object
      )
    ) {
    ...
    this.props.onPageChange(paging.pageIndex, this.state.filter);
    return false;
  };
