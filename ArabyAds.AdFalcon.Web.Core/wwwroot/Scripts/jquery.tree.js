...
(function ($) {
    ...
    $.tree = {
        ...
        defaults: {
            ...
            ui: {
                ...
                selected_parent_close: false,//OSaleh @ 12-01-2012 2:20 "select_parent", // false, "deselect", "select_parent"
        ...
        return {
            ...
            refresh: function (obj) {
                ...
                if (this.container.children("ul").length == 0) {
                    //Changed by Osaleh on 19-09-2011
                    ...
                    this.container.html("<ul class='ltr'><li class='last'><a class='loading' href='#'><ins>&nbsp;</ins>" + (this.settings.lang.loading || "Loading ...") + "</a></li></ul>");
                }
            ...
            reselect: function (is_callback) {
                ...
                this.container.children("ul:eq(0)").addClass("ltr");
            ...
            attach_events: function () {
                ...
                $("a", this.container.get(0))
					.on("click", function (event) { 
                        ...
                        // WHEN CLICK IS ON THE TEXT OR ICON
					...
					.on("dblclick", function (event) {
					    ...
					    // WHEN DOUBLECLICK ON TEXT OR ICON
					    if (_this.locked) {
					        event.preventDefault();
					        event.stopPropagation();
					        event.target.blur();
            ...
            off_height: function () {
                if (this.offset === false) {
                    ...
                    tmp = parseInt($(this.container.get(0)).css("paddingTop", 10), 10);
                    if (tmp) this.offset.top += tmp;
                    tmp = parseInt(  $(this.container.get(0)).css("borderTopWidth", 10)  , 10);
                    if (tmp) this.offset.top += tmp;
            ...
            destroy: function () {
                ...
                $("#" + this.container.attr("id")).off("click.jstree").off("dblclick.jstree").off("mouseover.jstree").off("mouseout.jstree").off("mousedown.jstree");
                //Added by Osaleh on 12-09-2011
                $("a", this.container.get(0)).off("click");
                $("li", this.container.get(0)).off("click");
    ...
    $(function () {
        ...
        var css = '/* TREE LAYOUT */ .tree ul { margin:0 0 0 5px; padding:0 0 0 0; list-style-type:none; } .tree li { display:block; min-height:18px; line-height:18px; padding:0 0 0 15px; margin:0 0 0 0; /* Background fix */ clear:both; } .tree li ul { display:none; } .tree li a, .tree li span { display:inline-block;line-height:16px;height:16px;color:black;white-space:nowrap;text-decoration:none;padding:1px 4px 1px 4px;margin:0; } .tree li a:focus { outline: none; } .tree li a input, .tree li span input { margin:0;padding:0 0;display:inline-block;height:12px !important;border:1px solid white;background:white;font-length:10px;font-family:Verdana; } .tree li a input:not([class="xxx"]), .tree li span input:not([class="xxx"]) { padding:1px 0; } /* FOR DOTS */ .tree .ltr li.last { float:left; } .tree > ul li.last { overflow:visible; } /* OPEN OR CLOSE */ .tree li.open ul { display:block; } .tree li.closed ul { display:none !important; } /* FOR DRAGGING */ #jstree-dragged { position:absolute; top:-10px; left:-10px; margin:0; padding:0; } #jstree-dragged ul ul ul { display:none; } #jstree-marker { padding:0; margin:0; line-height:5px; font-length:1px; overflow:hidden; height:5px; position:absolute; left:-45px; top:-30px; z-index:1000; background-color:transparent; background-repeat:no-repeat; display:none; } #jstree-marker.marker { width:45px; background-position:-32px top; } #jstree-marker.marker_plus { width:5px; background-position:right top; } /* BACKGROUND DOTS */ .tree li li { overflow:hidden; } .tree > .ltr > li { display:table; } /* ICONS */ .tree ul ins { display:inline-block; text-decoration:none; width:16px; height:16px; } .tree .ltr ins { margin:0 4px 0 0px; } ';
        if ($.browser.msie) {
		...
		"json" : function () {
			return {
				get		: function(obj, tree, opts) { 
					...
					if (obj.hasClass("locked")) json.data.state = "locked";
				...
				parse: function (data, tree, opts, callback) {
					...
					if(data.attributes) {
						for(var i in data.attributes) {
							...
							if(i == "class") {
								...
								if (data.state == "closed" || data.state == "open" || data.state == "locked") str += " " + data.state + " ";
								str += "' ";
								if (data.attributes[i] == "locked") {
								    locked = true;
								}
								cls = true;
							}
					...
					if (!cls && (data.state == "closed" || data.state == "open" || data.state == "locked")) str += " class='" + data.state + "' ";
					str += ">";
					...
					if(tree.settings.languages.length) {
						...
						attr["style"] = "";
						attr["class"] = "";
						if((typeof data.attributes).toLowerCase() != "undefined") {
							for(var i in data.attributes) {
								if(!data.attributes.hasOwnProperty(i)) continue;
								if(i == "style" || i == "class")	attr[i] += " " + data.attributes[i];
								else								attr[i]  = data.attributes[i];
							}
						}
						str += "<a";
						for (var i in attr) {
