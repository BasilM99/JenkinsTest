...
namespace ArabyAds.AdFalcon.Web.Controllers.Utilities.PaymentGateways
{
    class PayPalHelper : IPaymentGatewayHelper
    {
        ...
        public string RedirectToGateWayString(decimal amount, int transactionId)
        {
            ...
            requestTokenValues["returnurl"] = string.Format(PaymentGatewayDTO.ReturnPageUrl, Config.CurrentLanguage, transactionId,ApplicationContext.Instance.Tenant.Domain, JsonConfigurationManager.AppSettings["SslPort"].ToString());
            requestTokenValues["PAYMENTREQUEST_0_AMT"] = amount.ToString(CultureInfo.InvariantCulture);
            requestTokenValues["cancelurl"] = string.Format(requestTokenValues["cancelurl"], Config.CurrentLanguage, transactionId,ApplicationContext.Instance.Tenant.Domain, JsonConfigurationManager.AppSettings["SslPort"].ToString());
            ...
            {
                return  String.Format(PaymentGatewayDTO.Settings["set_paymenturl"], requestTokenResult["TOKEN"]);
            }
            else
            {
        ...
        public PaymentStatus CompletePayment(NameValueCollection queryStrings,ActionContext context)
        {
            FundTransactionDto fundDto = _fundTransactionService.GetFundTransactionById(new ValueMessageWrapper<int> { Value = int.Parse(queryStrings["transid"]) });
        ...
        private NameValueCollection CallIntegrationUrl(NameValueCollection values)
        {
            ...
            using (StreamReader reader = new StreamReader(request.GetResponse().GetResponseStream()))
            {
                ...
                var queryDictionary = Microsoft.AspNetCore.WebUtilities.QueryHelpers.ParseQuery(reader.ReadToEnd());

                return queryDictionary.ToNameValueCollection();
                //return HttpUtility.ParseQuery();
            }
        }
