...
{
    public class AdCreativeUnitMapping : ClassMap<ArabyAds.AdFalcon.Domain.Model.Campaign.AdCreativeUnit>
    {
        public AdCreativeUnitMapping()
        {
            ...
            HasMany(x => x.Trackers).KeyColumn("AdCreativeUnitId").Where(x => !x.IsDeleted).BatchSize(500).Cascade.All();
            HasOne(x => x.InStreamVideoCreativeUnit).Cascade.All();
            HasMany(p => p.AdCreativeUnitVendorList).KeyColumn("AdCreativeUnitId").Cascade.AllDeleteOrphan().BatchSize(500).Inverse();
    ...
    public class InStreamVideoCreativeUnitsMapping : ClassMap<ArabyAds.AdFalcon.Domain.Model.Campaign.InStreamVideoCreativeUnit>
    {
