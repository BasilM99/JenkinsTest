...
{
    //campaigns
    public class CampaignMapping : ClassMap<ArabyAds.AdFalcon.Domain.Model.Campaign.Campaign>
    {
        public CampaignMapping()
        {
           ...
           // Map(x => x.LifeTime).CustomType(typeof(CampaignLifeTime));
            ...
            HasMany(d => d.CampaignBidModifiers).KeyColumn("CampaignId").Cascade.All().Where("AdGroupId IS NULL");//.Where(x => !x.IsDeleted).Cascade.All();
