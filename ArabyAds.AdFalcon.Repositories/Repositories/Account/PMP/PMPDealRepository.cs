...
namespace ArabyAds.AdFalcon.Persistence.Repositories.Account.PMP
{
    public class PMPDealRepository : RepositoryBase<PMPDeal, int>, IPMPDealRepository
    {
        ...
        public IList<PMPDeal> GetAllPMPDeals(int AccountId, int AdvertiserAccountId)
        {
            IList<PMPDeal> PMPDeals = Query(x => (!x.IsDeleted && x.Account.ID == AccountId) ||(!x.IsDeleted && x.AdvertiserAccount != null && x.AdvertiserAccount.ID == AdvertiserAccountId) || (!x.IsDeleted &&  x.IsGlobal == true) || (/* Open Auction */ x.ID == 1)).ToList();
