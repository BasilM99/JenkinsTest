...
namespace Noqoush.AdFalcon.Web.Controllers.Controllers
{
    ...
    public class ReportsController : AuthorizedControllerBase
    {
        ...
        public ReportsController(IAudienceSegmentService  segmentService, IAppSiteService appSiteService, IMetricService metricService, IReportService reportService, ICampaignService campaignService, IAccountService accountService)
        {
              _AudienceSegmentService= segmentService;
        ...
        public ActionResult Index(string reportType)
        {
           ...
           if (!string.IsNullOrEmpty(Request.QueryString["id"]))
            {
              var Job=  _ReportService.GetSchadulingReport(Convert.ToInt32(Request.QueryString["id"]));
        ...
        public ActionResult GenerateCampaignChart(string metricCode, string fromdate, string toDate, string criteriaOpt, string AdsList, string advancedCriteria, string deviceCategory, string tabId, string groupByName, string AdvertiserId, string AccountAdvertiserId)
        {
        ...
        public ActionResult testGrid(string ColumnIds)
        {
            ...
            var col1 = _ReportService.GetColumn(1);
            var col2 = _ReportService.GetColumn(2);
            var col3 = _ReportService.GetColumn(14);
        ...
        public ActionResult GetAdvertiserItems(string type, int? id, int? AdvertiserId)
        {
            ...
            if(AdvertiserId.HasValue)
         {
            ...
            {
                case "ad":
                    adItems = _CampaignsService.GetAdsAdvTree(AdvertiserId).ToList();
                    break;
                case "adgroup":
                    adItems = _CampaignsService.GetAdGroupsAdvTree(AdvertiserId).ToList();
                    break;
                case "campaign":
                    adItems = _CampaignsService.GetCampaignsAdvTree(AdvertiserId).ToList();
                    break;
                case "devicemodel":
                    adItems = _CampaignsService.GetCampaignsAdvTree(AdvertiserId).ToList();
                    break;
        ...
        public ActionResult AppReportExport(FormCollection collection, string fromdate, string toDate, string summaryBy, string layout, string criteriaOpt, string AdsList, string advancedCriteria, string deviceCategory, string tabId, string page, string orderBy, string exportType, string metriceColumns)
        {
            List<AppCommonReportDto> reportingList;
            int counter;
        ...
        public string testReporSchduling()
        {
            ...
            dsdsd.TabId = "Opt";
            dsdsd.FromDate = Noqoush.Framework.Utilities.Environment.GetServerTime();
            dsdsd.ToDate = Noqoush.Framework.Utilities.Environment.GetServerTime().AddDays(-20);
            ...
            ReportSchedulerDto testob = new ReportSchedulerDto
            {
                ...
                AccountId = 87062,
                TimeSentAt = Noqoush.Framework.Utilities.Environment.GetServerTime().AddMinutes(10),
                ReportDto = dsdsd,
                StartDate = Noqoush.Framework.Utilities.Environment.GetServerTime(),
        ...
        {
            Noqoush.AdFalcon.Web.Controllers.Model.Report.Filter filter = new Model.Report.Filter();
