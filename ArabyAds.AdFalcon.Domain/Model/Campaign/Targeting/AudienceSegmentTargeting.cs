...
namespace ArabyAds.AdFalcon.Domain.Model.Campaign.Targeting
{
    ...
    {
        public List<Segment> UsedSegments { get; set; } = new List<Segment>();
        public List<Segment> BilledSegments { get; set; } = new List<Segment>();
        ...
        public static OperationResult MergeAnd(OperationResult left, OperationResult right)
        {
            ...
            for (int i = 0; i < left.BilledSegments.Count; i++)
            {
                ...
                Segment rseg = null;
                if(right!=null && right.BilledSegments!=null && right.BilledSegments.Count>0 && lseg!=null && lseg.Provider!=null)
                rseg = right.BilledSegments.Find((x) =>x!=null && x.Provider!=null&& x.Provider.ID == lseg.Provider.ID);
            ...
            if (right.UsedSegments!=null)
            {
                ...
                {
                    Segment lseg = null;
                    Segment rseg = right.UsedSegments[i];
                    if(rseg!=null)
                     lseg = left.UsedSegments.Find((x) => x!=null && x.Code == rseg.Code);
                    if (lseg == null)
                    {
        ...
        private static Segment MaxPrice(Segment seg1, Segment seg2)
        {
    ...
    public class AudienceSegmentTargeting : TargetingBase
    {
        ...
        public virtual bool CheckIfDataProviderCostElementAdded(AdGroup groupObj, string GroupStrRule)
        {
            try
            {
                ...
                foreach (var dpProv in dpProviders)
                {
                    ...
                    eachAdded = groupObj.SetAccountCostElmentsSavedForDataProv(Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().AccountId.Value, dpProviderAccounts[dpProv]);
        ...
        public virtual string getFirstPartyCode()
        {
        ...
        public virtual Rule getBinaryCondtionsForRules(child[] rules, OperatorRule op)
        {
            ...
            if (rules != null && rules.Length > 0 && rules[0].group == null )
            {
                int? recency2 = null;
            ...
            else  if(rules != null && rules.Length > 0 &&  rules[0].group!=null)
        ...
        public virtual RuleVersion GetRulesForExpression(int lastVersion = 0, bool UseOrg = false)
        {
            ...
            if (obj.rules != null && obj.rules.Length > 0)
            {



                if (obj.rules[0].group == null && obj.rules.Length == 1)
                {

                    obj.rules = null;
                }
                if (obj.rules[0].group == null && obj.rules.Length == 0)
                {

                    obj.rules = null;
                }
                foreach (var rulgr in obj.rules)
                {


                    if (rulgr.group != null && rulgr.group.rules != null)
                    {
                        if (rulgr.group.rules.Length == 0)
                        {
                            rulgr.group = null;
                        }
                    ...
                    if (rulgr.group != null && rulgr.group.rules != null)
                    {
                        if (rulgr.group.rules.Length == 1)
                        {
                            if (string.IsNullOrWhiteSpace(rulgr.group.rules[0].id))
                            {
                                rulgr.group = null;
                            }
                        }
            ...
            obj.rules = obj.rules.Where(x => x.group != null && x.group.rules!=null && x.group.rules.Any(x=>!string.IsNullOrWhiteSpace(x.id)) ).ToArray();
            if (obj.rules != null && obj.rules.Length > 0)
            {
              var objR=  getBinaryCondtionsForRules(obj.rules, op);
                if (objR != null)
                    rversion.Version_1 = objR;
                else
                    return null;
            }
            else
        ...
        public virtual bool CalculateBillInternalMin(Rule node, Stack<OperationResult> operationResults)
        {
            ...
            {
                AudienceSegment userSegmentDefinition = null;
                 match = true;
                if(node!=null && node.Segment>0)
                 userSegmentDefinition = AudienceSegmentRepository.GetSegmentByCode(node.Segment);
                else
                    match = false;
                //if (node.Not)
                //{
        ...
        public virtual void PublishkafkaforCheck(List<int> Ids,int campId , string Action)
        {
            ...
            //{
                ArabyAds.AdFalcon.Domain.Configuration.KafkaEventPublisher.Publish(new AudienceListCheck { Ids = Ids, CampaignId= campId, Action= Action });
    ...
    public class ContextualSegmentTargeting : TargetingBase
    {
        public ContextualSegmentTargeting()
        {

        }

        public virtual bool Include { get; set; }
        public virtual bool IsBrandSafty { get; set; }

        public virtual ContextualSegment ContextualSegment { get; set; }


        public static IContextualSegmentRepository AudienceSegmentRepository = IoC.Instance.Resolve<IContextualSegmentRepository>();
        public static ICampaignRepository CampaignRepository = IoC.Instance.Resolve<ICampaignRepository>();
        public static IAccountCostElementRepository AccountCostElementRepository = IoC.Instance.Resolve<IAccountCostElementRepository>();
        private string codeForFirstParty = "fpcd";
     
        public ContextualSegmentTargeting(string RulesJson)
        {
            this.RulesJson = RulesJson;
        }
       ...
       public virtual string RulesJson { get; set; }
        ...
        public override TargetingBase Copy()
        {
            var cloneObj = new ContextualSegmentTargeting()
            {
                ContextualSegment = this.ContextualSegment,
               // RulesJson = this.RulesJson,
                AdGroup = this.AdGroup,
                Type = this.Type,
                Include = this.Include,
                IsBrandSafty = this.IsBrandSafty,
        ...
        public virtual string getFirstPartyCode()
        {
            ...
            if (JsonConfigurationManager.AppSettings["codeForFirstPartyContextual"] != null && !string.IsNullOrEmpty(JsonConfigurationManager.AppSettings["codeForFirstPartyContextual"]))
            {
                ...
                codeForFirstParty = JsonConfigurationManager.AppSettings["codeForFirstPartyContextual"];
        ...
        public virtual RuleVersion GetRulesForExpression(int lastVersion = 0, bool UseOrg = false)
        {
            ...
            obj.rules = obj.rules.Where(x => x.group != null).ToArray();
            if (obj.rules != null && obj.rules.Length > 0)
                rversion.Version_1 = getBinaryCondtionsForRules(obj.rules, op);
            else
                return null;
        ...
        public virtual List<ContextualSegmentTargeting> GetContextualObject(string groupJson)
        {
            List<ContextualSegmentTargeting> contextualSegmentTargetingList = new List<ContextualSegmentTargeting>();
            if (!string.IsNullOrEmpty(groupJson))
            {
                group deserializedGroupJson = JsonConvert.DeserializeObject<group>(groupJson);
                foreach (var item in deserializedGroupJson.rules)
                {
                    var groupItem = item.group;
                    if (groupItem != null)
                    {
                        foreach (var rulegroup in groupItem.rules)
                        {
                            if (rulegroup != null)
                            {
                                ...
                                contextualSegmentTargeting.ContextualSegment = AudienceSegmentRepository.Get(Convert.ToInt32(rulegroup.id));
                                if (rulegroup.condition == "Target")
                                    contextualSegmentTargeting.Include = true;
                                else
                                    contextualSegmentTargeting.Include = false;

                                contextualSegmentTargetingList.Add(contextualSegmentTargeting);
                            }
                        }
                    }
                }
            ...
            return contextualSegmentTargetingList;
        }

        public virtual void SaveContextualSegmentTargeting(AdGroup adGroup, string groupContextualString, TargetingType Type, bool IsBrandSafty=false)
        {
            var campaign = adGroup.Campaign;

            if (!string.IsNullOrEmpty(groupContextualString))
            {
                var contextualObjectList = GetContextualObject(groupContextualString);
                if (contextualObjectList.Count > 0)
                {
                    foreach (var contextualObject in contextualObjectList)
                    {
                        contextualObject.Type = Type;
                        contextualObject.IsBrandSafty = IsBrandSafty;
