...
namespace ArabyAds.AdFalcon.Domain.Model.Campaign
{
    public class AdCreative : AdBase<AdCreative, AdCreativeStatus>
    {
        ...
        public virtual bool PublishAdStatusInforForKafkaReSubmit(EntityEventData args)
        {
            ...
            if (isDirty)
            {
                ...
                foreach (var item in StatusInfoPropTobeNotifyed)
                {
                    ...
                    {
                        var status = (AdCreativeStatus)(args.State[index]);
                        var oldstatus = (AdCreativeStatus)(args.OldState[index]);
                        if ((oldstatus != null && oldstatus.ID == AdCreativeStatus.ActiveAdServer.ID &&  status != null && status.ID == AdCreativeStatus.Active.ID) ||(oldstatus != null && oldstatus.ID == AdCreativeStatus.ActiveAdServer.ID &&  status != null && status.ID == AdCreativeStatus.Submitted.ID))
                            return true;
        ...
        protected virtual void UploadSnapshots()
        {
            ...
            foreach (var adCreativeUnit in AdCreativeUnits.Where(p => p.SnapshotDocument != null))
            {
                ...
                {
                    string url = adCreativeUnit.SnapshotDocument.ftpUpload(directory, cdnUrl,c++);
