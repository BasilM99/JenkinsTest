...
namespace ArabyAds.AdFalcon.Domain.Model.Account
{
    public class Account : Party//IEntity<int>
    {
        ...
        {
            if (agC == ArabyAds.AdFalcon.Domain.Common.Model.Account.AgencyCommission.Undefined)
            {
        ...
        {
            if (this.AccountSummary.Earning < payment.Amount)
            {
        ...
        public virtual Core.Discount GetActiveDiscount()
        {
            ...
            if (this.Discounts!=null&& this.Discounts.Count>0)
            {
                ...
                {
                    discount = new Core.Discount { FromDate= accountDiscount.Discount.FromDate, Value = accountDiscount.Discount.Value, Type = Common.Model.Core.DiscountType.Percentage };//, ToDate =(Discount.ToDate };
                }
            }
            return discount;
        }
        public virtual void GoDSP(bool activate = true)
        {
            ...
            {
                //by mosab 
                _securityService.ActivateUser(ValueMessageWrapper<string>.Create(this.PrimaryUser.EmailAddress));
        ...
        {
            ApplicationContext.CreateContext(this.Tenant.Name, false, new Tenant { Name = this.Tenant.Name, ID = this.Tenant.ID, Domain = this.Tenant.Domain, Code = this.Tenant.Code });
