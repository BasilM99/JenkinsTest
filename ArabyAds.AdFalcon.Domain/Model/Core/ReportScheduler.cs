...
namespace ArabyAds.AdFalcon.Domain.Model.Core
{
    ...
    public class ReportScheduler : IEntity<int>
    {
        ...
        private string GetCampaignNameForQ(int campaignId)
        {
        ...
        private string GetAdGroupNameForQ(int Id)
        {

            var name = AdGroupRepository.Query(M => M.ID == Id).Select(M => M.Name).SingleOrDefault<string>(); ;
            return name;
        }
        private string GetAdNameForQ(int Id)
        {

            var name = AdCreativeRepository.Query(M => M.ID == Id).Select(M => M.Name).SingleOrDefault<string>(); ;
            return name;
        }
        ...
        public virtual string GetCompositeMame(string ItemsList, string TabId, bool isEmail, string queryJsonData)
        {
            string Name = "";
            ...
            if (Ids.Count == 1 && string.IsNullOrEmpty(this.PreferedName) && !this.IsForQueryBuilder)
            {
            ...
            if (this.IsForQueryBuilder)
            {

                string copqueryJsonData = queryJsonData;
                ...
                if (Querydata != null)
                {
                    ...
                    foreach (KeyValuePair<string, string> entry in Querydata)
                    {
                        ...
                        if (TagIds != null && TagIds.Count == 1)
                        {
                            ...
                            {
                                substitutionName = GetAdNameForQ(TagIds[0]);
                                Name = Name.Replace("{EntityName}", ResourceManager.Instance.GetResource("AdSubject", "Report") + substitutionName);
                                break;
                            }
                            else if (int.Parse(entry.Key) == 2)
                            {
                                substitutionName = GetAdGroupNameForQ(TagIds[0]);
                                Name = Name.Replace("{EntityName}", ResourceManager.Instance.GetResource("AdGroupSubject", "Report") + substitutionName);
                                break;
                            }
                            else if (int.Parse(entry.Key) == 1)
                            {
                                substitutionName = GetCampaignName(TagIds[0]);
                                Name = Name.Replace("{EntityName}", ResourceManager.Instance.GetResource("CampaignSubject", "Report") + substitutionName);
                                break;
                            ...
                            {
                                substitutionName = GetAppSiteName(TagIds[0]);
                                Name = Name.Replace("{EntityName}", ResourceManager.Instance.GetResource("AppSiteSubject", "Report") + substitutionName);
                                break;
                            }
            ...
            if (!string.IsNullOrEmpty(this.PreferedName) && !this.IsForQueryBuilder)
            {
                ...
                if (!string.IsNullOrEmpty(this.PreferedName) && this.IsForQueryBuilder && string.IsNullOrEmpty(substitutionName))
                {
                    substitutionName = this.PreferedName;
                    Name = Name.Replace("{EntityName}", this.PreferedName);
