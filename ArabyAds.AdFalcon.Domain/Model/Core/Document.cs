       ...
       static WebHDFSUtil _WebHDFSUtil = null;
        ...
        private static WebHDFSUtil WebHDFSUtil
        {
            get
            {
                ...
                {
                    _WebHDFSUtil = new WebHDFSUtil(JsonConfigurationManager.AppSettings["LogImpPath"], JsonConfigurationManager.AppSettings["LogImpPath2"], "", new NetworkCredential(JsonConfigurationManager.AppSettings["WebHDFSUserName"], JsonConfigurationManager.AppSettings["WebHDFSPassword"], JsonConfigurationManager.AppSettings["WebHDFSDomain"]));
                }
                return _WebHDFSUtil;
            }
        }
        ...
        public virtual void ftpUpload(string ftpUrl)//, string cdnUrl)
        {
            ...
            {
                //delete old file
                Framework.Utilities.CDNHelper.Delete(ftpUrl);
            }
            Framework.Utilities.CDNHelper.Upload(ftpUrl, ReadContent());
        }

        public virtual string ftpUpload(string directory, string cdnUrl)
        {
            //directory += "/";
            //Framework.Utilities.CDNHelper.CreateDirectory(directory);

            //directory += subDirectory + "/";
            //Framework.Utilities.CDNHelper.CreateDirectory(directory);
            var fileName = FullNameUpdate().Replace(" ", "_");
            Framework.Utilities.CDNHelper.Upload(directory + "/" + fileName, ReadContent());
            return (cdnUrl + "/" + fileName);
        }

        public virtual string ftpUpload(string directory, string cdnUrl,int c)
        {
            //directory += "/";
            //Framework.Utilities.CDNHelper.CreateDirectory(directory);
           ...
           string FullNameUpdateStr = string.Format("{0}{2}{1}", this.GetNameWithNoExtension(), this.Extension,c); 
            var fileName = FullNameUpdateStr.Replace(" ", "_");
            Framework.Utilities.CDNHelper.Upload(directory + "/" + fileName, ReadContent());
            return (cdnUrl + "/" + fileName);
        }
        ...
        public virtual string StructureTheName(string nameWithExtnsion)
        {
            ...
            return JsonConfigurationManager.AppSettings["WebHDFSDirectory"] + "/" + subFolder + "/" + Configuration.Id64Generator.GenerateId().ToString()+"/"+ nameWithExtnsion; 
