...
using Noqoush.Framework.DistributedEventBroker.PubSub;
using System;
...
namespace Noqoush.AdFalcon.AdFalconPortalMaintenanceJob.Utilities.Events
{
    public static class KafkaEvents
    {
        ...
        public static void CampaignInvoiced(CampaignOverspend evt)
        {
            try
            {
                ...
                if (evt.Amount > 0)
                {
                    SchedulerHelper.FundTransactionService.AddOverBudgetReturnFundFromCampaign(Convert.ToInt32(evt.Id), evt.Amount);
        ...
        public static void CampaignBillingInfoChangedProcessed(CampaignBillingInfoChangedAck evt)
        {
            ...
            {
                SchedulerHelper.FundTransactionService.SendCampaignbillingInfoacknowledgment(evt.Id, "Budget", evt.RequestedAmount, evt.CommittedAmount, evt.ModifiedOn);
        ...
        public static void CampaignStatusChangedProcessed(CampaignStatusChanged evt)
        {
            ...
            try
            {
                SchedulerHelper.EmailHandlerService.HandelEvent(new Framework.EventBroker.EventArgsBase(GetEventsString(evt.NewStatus), evt.CampaignId.ToString()));
            }
            catch (Exception ex)
            {
           ...
           try
            {
                ...
                SchedulerHelper.MessagesEventBrokerService.HandelEvent(new Framework.EventBroker.EventArgsBase(GetEventsString(evt.NewStatus), evt.CampaignId.ToString()));
        ...
        public static void CampaignFundAboutTobeConsumedProcessed(CampaignFundAboutTobeConsumed evt)
        {
            try
            {
                ...
                SchedulerHelper.EmailHandlerService.HandelEvent(new Framework.EventBroker.EventArgsBase("FundAboutTobeConsumed", evt.CampaignId.ToString()));
        ...
        public static void AudienceListProcessed(AudienceListCheck  evt)
        {
            try
            {
                ...
                ars.Add(ids);
                SchedulerHelper.MessagesEventBrokerService.HandelEvent(new Framework.EventBroker.EventArgsBase("AudienceListCheck", evt.CampaignId.ToString(),null,ars));
