        ...
        public void ResolvePendingTransactions()
        {
            try
            {
                ...
                foreach (var trans in pendingTrans.Select(p=>p as PgwFundTransactionResponseDto))
                {
                    ...
                    if (trans.TransactionDate < Noqoush.Framework.Utilities.Environment.GetServerTime().AddDays(-3))
                    {
                        Noqoush.Framework.ApplicationContext.Instance.Logger.Warn(string.Format("The TransactionID: {0}, are not checked as passed 3 days and it will not be in QueryDR please contact Admin", trans.ID));
                        continue;
                    }
                    string responseText;
