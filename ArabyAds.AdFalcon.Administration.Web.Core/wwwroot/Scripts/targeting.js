...
function localInitilize() {
    ...
    function CheckAppSiteNotCompatibleWithCampaign() {
        ...
        if (grid) {
            if ($('#InserteCampaignBidConfigs').val().length>0)
                Data.InsertedItems = $('#InserteCampaignBidConfigs').val();
...
function checkStatus(IsContinue) {
    ...
    result = checkBid() && result && !disableSaveButtonForAjax && ValidatedIPRanges() && BudgetChanged() && validateConversions() && CheckBidModifierDuplication();
...
function getGeoFencingTargeting() {
    var grid = $('#GeoFencingsGrid').data("kendoGrid");
    if (grid == null)
        return;
    ...
    if (InsertedIds.length > 0)
        $('#InsertedGeoFencing').val(jQuery.toJSON(inserted));
    else
        $('#InsertedGeoFencing').val('');
    $('#DeletedGeoFencing').val(deletedIPRangeIds);
...
function getIPRanges() {
    ...
    if (InsertedIds.length > 0)
        $('#InsertedIPRanges').val(jQuery.toJSON(inserted));
    else
...
function getTrackingEvents() {
    ...
    for (var idx = 0; idx < datads.length; idx++) {
        ...
        deletedIPRangeIds += '&' + datads[idx].Id;
        deletedIPRangeIdsCode += '&' + datads[idx].Code;
    }


    if (InsertedIds.length > 0)
        $('#InsertedTrackingEvents').val(jQuery.toJSON(inserted));
    else
        $('#InsertedTrackingEvents').val('');
    $('#DeletedTrackingEvents').val(deletedIPRangeIds);

    $('#DeletedTrackingCodeEvents').val(deletedIPRangeIdsCode);
...
function getURLTargeting() {
    ...
    if (InsertedIds.length > 0)
        $('#InsertedURLTargeting').val(jQuery.toJSON(inserted));
    else
        $('#InsertedURLTargeting').val('');
    $('#DeletedURLTargeting').val(deletedIPRangeIds);
