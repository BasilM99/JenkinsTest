...
const Campaigns = ({ t, rtl }) => {
    ...
    const tempCampId =tempUrlParts[tempUrlParts.length - 2];
    const adGroupId = tempUrlParts[tempUrlParts.length - 1];
    ...
    const [state , setLocalState] = useState({
        ...
        ConfirmMessage:t("Ad:SelectConfirmation"),
        selectedIds:[],
        filters:{
    ...
    const actoinButtonClick = (type) => {
        ...
        //var baseUrl = Constants.backend.getAdByGroupId;

        let baseUrl = Constants.backend.getAdByGroupId.replace("{0}",tempCampId);
        baseUrl = baseUrl.replace("{1}",tempCampId);
    ...
    const collectSelectedCampaigns = (items) =>{
        if(items.length > 0)
        {
            let tempCampaigns = items.map(x => {
             ...
             tempMess = tempMess.replace("{0}",t('Titles:Ads')) ;
            ...
            state.ConfirmMessage =t("Ad:SelectConfirmation");
            state.selectedIds = [];
            state.color='danger';
            setLocalState({...state});
        }
    ...
    const contextMenu =(props) =>{
        let EditUrl = "/Campaign/Creative/"+ tempCampId+"/"+adGroupId+"/" + props.dataItem.Id;
        let ReportsUrl = "/Reports?reportType=ad&itemId=" + props.dataItem.Id;
        let AuditTrailUrl = "/Campaign/RedirectToAuditTrial/" + tempCampId+"/" + props.dataItem.Id;
        ...
        return <>
            ...
            <CloneComponent title={t('Commands:Copy')}   name={props.dataItem.Name} t={t} cloneUrl={Constants.backend.cloneAd  +"?id="+  tempCampId+"&adGroupId="+ adGroupId+"&adId="+props.dataItem.Id} cloneSuccessMessage={t("Clone:Ad")} cloneErrorMessage={t("Errors:CloneAdError")}/>
    ...
    const renderGroupNameLink = (props) =>{
        let tmpUrl =  "/Campaign/Creative/"+ tempCampId+"/"+adGroupId+"/" + props.dataItem.Id;
    ...
    const renderCampaignsComponent = () =>{
        ...
        let url  =Constants.backend.getAdsDataByGroupId;
        
        //let tempCampId = location.pathname.substr(location.pathname.lastIndexOf('/')+1,location.pathname.length);
        //url += tempUrlParts;
        
        url = url.replace("{0}",tempCampId);
        url = url.replace("{1}",adGroupId);
        //pageChange();
        return(
            <Container className="dashboard">
                ...
                <Col md={12}>
                    ...
                    <h3 className="page-title">{t('Titles:Ads')}</h3>
                </Col>
                </Row>
                    ...
                    <Card>
                        <CardBody  >
                            <div className="card__title">
                                ...
                                <h5 className="bold-text">{t('Titles:Ads')}</h5>
                            </div>
                ...
                <Row>
                                ...
                                <Col  md={12} lg={4} xl={2}>
                                    ...
                                    <Button color="success" className=" p-2"><a className="text-white" href={"/Campaign/Creative/"+tempCampId+"/"+adGroupId} >{t("Commands:AddNewAds")}</a></Button>
