...
using ArabyAds.Framework.DB;

namespace ArabyAds.AdFalcon.Persistence.Reports.Repositories.PetaPoco
{
    ...
    public interface IMapper
    {
        ...
        Func<object, object> GetToDbConverter(System.Type SourceType);
    }
    ...
    public class Database : IDisposable
    {
        ...
        public Database(string connectionString, string providerName)
        {
            ...
            _providerTypeName = providerName;
            CommonConstruct();
        }
        ...
        public Database(string connectionStringName)
        {
            ...
            if (connectionStringName == "")
                connectionStringName = JsonConfigurationManager.ConnectionStrings.Keys[0];

            // Work out connection string and provider name
            var providerName = "System.Data.SqlClient";
            if (!string.IsNullOrEmpty(JsonConfigurationManager.ConnectionStrings[connectionStringName]))
            {
                _factory = DatabaseFactory.GetDbProviderFactory(connectionStringName);
                if (_factory != null)
                    providerName = _factory.GetType().Name;
            }
            else
            {
            ...
            _connectionString = JsonConfigurationManager.ConnectionStrings[connectionStringName];
            _providerTypeName = providerName;
            CommonConstruct();
        }
        ...
        private void CommonConstruct()
        {
            ...
            //    _factory = DbProviderFactories.GetFactory(_providerTypeName);
            ...
            else if (_providerTypeName.IndexOf("MySql", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.MySql;
            else if (_providerTypeName.IndexOf("SqlServerCe", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.SqlServerCE;
            else if (_providerTypeName.IndexOf("Npgsql", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.PostgreSQL;
            else if (_providerTypeName.IndexOf("Oracle", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.Oracle;
            else if (_providerTypeName.IndexOf("SQLite", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.SQLite;
        ...
        public class PocoData
        {
            ...
            {
                // Check cache
                // Fix from Malik Hassan in 18/08/2014, use column names in the key instead of columns count
                // to fix bug when same column count but different columns names from database have been retrieved
                var columnsNames = new List<string>();
                for (int i = 0; i < r.FieldCount; i++)
                {
                    columnsNames.Add(r.GetName(i));
                }

                var key = string.Format("{0}:{1}:{2}:{3}:{4}", sql, connString, ForceDateTimesToUtc, firstColumn, string.Join("_", columnsNames));
                RWLock.EnterReadLock();
                try
                {
