...
{
    public class PerformanceReportRepository : BaseReportRepository, IPerformanceReportRepository
    {
        public PerformanceReportRepository(RepositoryImplBase<ChartDto, int> repository, ArabyAds.Framework.ConfigurationSetting.IConfigurationManager configurationManager)
        ...
        private List<BaseAppSitePerformanceDetailsDto> GetResult(PerformanceReportGeneratorArgs args)
        {
            ...
            Framework.ApplicationContext.Instance.Logger.Error(script);
            var items = GetResult<BaseAppSitePerformanceDetailsDto>(script).ToList();
