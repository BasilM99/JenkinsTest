...
using ArabyAds.AdFalcon.Exceptions.Fund;
...
namespace ArabyAds.AdFalcon.Services.Services.Account.Fund
{
    public class FundTransactionService : IFundTransactionService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public FundTransactionDto GetFundTransactionById(ValueMessageWrapper<int> id)
        {
            var item = _fundTransRepository.Get(id.Value);
        ...
        public void UpdateFundTransactionByref(UpdateFundTransactionByrefRequest request)
        {
            var item = _fundTransRepository.Get(request.Id);
            CheckAccountFundTransHistory(item);
            item.TransactionId = request.ReferenceId;
            _fundTransRepository.Save(item);
        }
        public ValueMessageWrapper<int> InitiateFundTransaction(FundTransactionDto data)
        {
            ...
            switch (data.FundTransType.ID)
            {
                case (int)AccountFundTransTypeIds.CreditCard:
                    fundTrans = MapperHelper.Map<AccountFundTransHistoryPgw>(data);
                ...
                case (int)AccountFundTransTypeIds.PayPal:
                    fundTrans = MapperHelper.Map<AccountFundTransHistoryPaypal>(data);
                    fundTrans.OriginalAmount = data.Amount;
                    fundTrans.Currency = Domain.Model.Core.Currency.USD;
                    (fundTrans as AccountFundTransHistoryPaypal).SystemPaymentDetail = Domain.Model.Core.SystemPaymentDetails.PayPal;
                    break;
                default:
                    fundTrans = MapperHelper.Map<AccountFundTransHistory>(data);
                    break;
            }


            fundTrans.AccountFundType = _accountFundTypeRepository.Get(data.AccountFundType.ID);
            ...
            return  ValueMessageWrapper.Create(fundTrans.ID);
        }
        ...
        /// <returns></returns>
        public ValueMessageWrapper<bool> CloseFundTransaction(FundTransactionResponseDto fundTransactionDTO)
        {
            ...
            {
                case (int)AccountFundTransTypeIds.CreditCard:
                    MapperHelper.Map<PgwFundTransactionResponseDto, AccountFundTransHistoryPgw>(fundTransactionDTO as PgwFundTransactionResponseDto, transHist as AccountFundTransHistoryPgw);
                    break;
                default:

                    MapperHelper.Map<PayPalFundTransactionResponseDto, AccountFundTransHistoryPaypal>(fundTransactionDTO as PayPalFundTransactionResponseDto, transHist as AccountFundTransHistoryPaypal);
                    break;
            }
            ...
            return ValueMessageWrapper.Create(transHist.FundTransStatus.ID == AccountFundTransStatus.Committed.ID);
        }
        ...
        {
            var result = (from pgw in _pgwRepository.Query(x => x.IsDeleted == false) select MapperHelper.Map<PgwDto>(pgw)).ToList();
            return result;
        }
        ...
        /// <returns></returns>
        public IList<FundTransactionResponseDto> GetPendingFundTransactions(GetPendingFundTransactionsRequest request)
        {
            var result = _fundTransRepository.Query(x => x.FundTransStatus.ID == (int)AccountFundTransStatusIds.Pending && x.TransactionDate <= request.DateTo && x.FundTransType.ID == request.fundTransactionTypeId).ToList().OrderByDescending(p => p.CreationDate);

            if (request.fundTransactionTypeId == AccountFundTransType.CreditCard.ID)
            {
        ...
        /// <returns></returns>
        public PgwDto GetPgwInfo(ValueMessageWrapper<int> id)
        {
            return MapperHelper.Map<PgwDto>(_pgwRepository.Get(id.Value));
        }
        ...
        public PgwDto GetPgwInfoByCode(string code)
        {
            ...
            return MapperHelper.Map<PgwDto>(fundPgw);
        }

        public void AddFund(NewFundDto fundDto)
        {
            var error = ValidateNewFundDto(fundDto);
        ...
        public void AddOverBudgetReturnFundFromCampaign(AddOverBudgetReturnFundRequest request)
        {

            var Campaign = _CampaignRepository.Get(request.Id);

            if (request.InvoiceAmount > 0)
            {
                request.InvoiceAmount = request.InvoiceAmount * 1;

            }
            if (Campaign != null)
            {
                Campaign.Account.SetTenant();
                NewFundDto fundDto = new NewFundDto();
                fundDto.AccountId = Campaign.Account.ID;
                fundDto.Amount = request.InvoiceAmount;
                fundDto.TransactionDate = Framework.Utilities.Environment.GetServerTime();
                fundDto.ObjectRelatedId = request.Id;
        ...
        public void AddOverBudgetReturnFundFromAccount(AddOverBudgetReturnFundRequest request)
        {
            ...
            if (account != null)
            {
                ...
                var firstFund = _fundTransRepository.Query(M => M.FundTransType.ID == AccountFundTransType.OverBudgetRefund.ID  && M.AccountId == request.Id && M.CreationDate >= DateTime.UtcNow.AddMonths(-1)).FirstOrDefault();
                account.AddFund(fund.Amount);
                _accountRepository.Save(account);
                if (firstFund == null)
                {
        ...
        private void SendEmail(string resourcekey, Dictionary<string, string> values)
        {
            ...
            _mailSender.SendEmail("", Domain.Configuration.NoReplyTenant, values["To"], values["To"], values["Subject"], emaildbnody, true, Domain.Configuration.EmailAdminTenant);
        }
        ...
        public void SendCampaignbillingInfoacknowledgment(SendBillingInfoacknowledgmentRequest request)
        {

            var Campaign = _CampaignRepository.Get(request.Id);

            if (Campaign != null && (Campaign.ModifiedOn == request.ModifiedOn))
            {
                Campaign.Account.SetTenant();
                ...
                values.Add("@FieldToChange", request.FieldToChange);
                values.Add("@RequestedAmount", request.RequestedAmount.Value.ToString("F2"));
                values.Add("@CommittedAmount", request.CommittedAmount.Value.ToString("F2"));
                ...
                Campaign.Budget = request.CommittedAmount.Value;
        ...
        public void SendAdGroupbillingInfoacknowledgment(SendBillingInfoacknowledgmentRequest request)
        {

            var AdGroup = _adGroupRepository.Get(request.Id);
            var campobj= _CampaignRepository.Get(AdGroup.Campaign.ID);
            if (AdGroup != null && (AdGroup.ModifiedOn == request.ModifiedOn))
            {
                AdGroup.Campaign.Account.SetTenant();
                Dictionary<string, string> values = new Dictionary<string, string>();
                ...
                AdGroup.Budget = request.CommittedAmount;
