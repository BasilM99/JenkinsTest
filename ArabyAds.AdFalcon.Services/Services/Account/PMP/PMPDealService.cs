...
namespace ArabyAds.AdFalcon.Services.Services.Account.PMP
{
    ...
    public class PMPDealService : IPMPDealService
    {
      ...
      IFloorPriceRepository floorPriceRepository,
        ...
        #region  Query Data
        public ResultPMPDealDto QueryByCratiriaForPMPDeal(ArabyAds.AdFalcon.Domain.Common.Repositories.Account.PMP.PMPDealCriteria wcriteria)
        {
       ...
       // private static Id64Generator _id64Generator = new Id64Generator(int.Parse(JsonConfigurationManager.AppSettings["HostId"]));
        public PMPDealSaveDto SavePMPDeal(PMPDealDto dto)
        {
            ...
            else
            {
                ...
                item.UniqueId = ArabyAds.AdFalcon.Domain.Configuration.Id64Generator.GenerateId();
            }
        ...
        public PMPDealDto GetDealPMPDeal(ValueMessageWrapper<int> Id)
        {
            var item = new PMPDeal();
            if (Id.Value > 0)
            {
                item = _PMPDealRepository.Get(Id.Value);
                ValidateDeal(item);
                var dto = MapperHelper.Map<PMPDealDto>(item);
                dto.PMPTargetingSaveDto = GetTargeting(Id.Value, item);
                return dto;
        ...
        public IList<PMPDealDto> GetAllPMPDealsByAccount(ValueMessageWrapper<int> AccountId)
        {

            var Deals = _PMPDealRepository.GetAllPMPDealsByAccount(AccountId.Value);
        ...
        public IList<DropDownDto> GetAllPMPDealsByUserAndAdvertiser(ValueMessageWrapper<int> AdvertiserAccountId)
        {
            ...
            var Deals = _PMPDealRepository.GetAllPMPDeals(AccountId, AdvertiserAccountId.Value);
            IList<PMPDeal> allDeals = new List<PMPDeal>();
            var dealManager = IsManager();
            foreach (var deal in Deals)
            {
                if(deal.IsGlobal)
                {
                    if(!deal.Name.Contains("-Global"))
                        deal.Name = deal.Name + "-Global";
            ...
            var list = allDeals.Select(Deal => new DropDownDto { Id = Deal.ID, Name = Deal.Name }).ToList();
            return list;
        }
        public IList<CampaignDto> getCampsBydeal(ValueMessageWrapper<int> dealid)
        {

            var Campaigns = _PMPDealRepository.getCampsBydeal(dealid.Value);
        ...
        public CampaignListResultDto getCampsAdvertiserBydeal(GetCampsAdvertiserBydealRequest request)
        {


            CampaignListResultDto results = new CampaignListResultDto();

            var Campaigns = _PMPDealRepository.getCampsBydeal(request.DealId);
            if (request.AdvertiserId > 0)
            {
                Campaigns = Campaigns.Where(M => M.AdvertiserAccount.ID == request.AdvertiserId).ToList();
            }
            if (Campaigns!=null)
            {
        ...
        public IList<AdvertiserAccountDto> getAdvertiserAccountsBydeal(ValueMessageWrapper<int> dealid)
        {

            var Campaigns = _PMPDealRepository.getAdvertiserAccountsBydeal(dealid.Value);
        ...
        public IList<AdGroupDto> getDealCampsAdgruops(GetDealCampsAdgroupsRequest request)
        {

            var Adgruops = _PMPDealRepository.getDealCampsAdgruops(request.DealId, request.CampaignId);
