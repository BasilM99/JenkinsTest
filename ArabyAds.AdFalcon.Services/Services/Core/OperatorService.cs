...
namespace ArabyAds.AdFalcon.Services.Services.Core
{
    public class OperatorService : IOperatorService
    {
        ...
        public IEnumerable<TreeDto> GetAllCountryOperator()
        {
            ...
            var list = operatorRepository.GetAll();
            var items = (from operater in list where operater.Location !=null && operater.Location.Name != null group operater by operater.Location.ID into g select new { Id = g.Key, Operators = g.ToList() }).ToList();
