...
namespace ArabyAds.AdFalcon.Services.Services.Campaign
{
    public class AdvertiserService : IAdvertiserService
    {
        ...
        private IContextualSegmentRepository _ContextualSegmentRepository = null;
        ...
        public AdvertiserService(IAdvertiserRepository AdvertiserRepository, IConfigurationManager configurationManager, IAdvertiserAccountRepository AdvertiserAccountRepository, ICampaignRepository CampaignRepository, ISummaryRepository summaryRepository
            ...
            IDSPAccountSettingRepository DSPAccountSettingRepository,
                   ...
                   IAudienceSegmentOccupationRepository AudienceSegmentOccupationRepository,
                    ...
                    IlookalikejobRepository lookalikejobRepository,IContextualPartnerRepository contextualPartnerRepository
            )
        {
        ...
        public AdvertiserDto Get(ValueMessageWrapper<int> id)
        {
            var Advertiser = AdvertiserRepository.Get(id.Value);
            if (Advertiser != null)
            {
        ...
        public AdvertiserAccountListResultDto GetAccountAdvertiser(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.AdvertiserAccountCriteria wcriteria)
        {
            ...
            if (criteria.Page.HasValue)
            {
                ...
                foreach (var advertiserListDto in returnList)
                {
                    ...
                    if (!advertiserListDto.IsDeleted)
                    {
                        ...
                        advertiserListDto.StatusString = _summaryRepository.CalculateAdvertiserStatusString(advAds);

                    }
                    else
                    {
                        advertiserListDto.Status = Framework.Resources.ResourceManager.Instance.GetResource("StatusNotActive", "PMPDeals");
                        advertiserListDto.StatusString = "NotActive";
                    }
                }
        ...
        public AdvertiserAccountListDto GetAccountAdvertiserById(ValueMessageWrapper<int> Id)
        {
            ...
            var item = _advertiserAccountRepository.Get(Id.Value);

            var returnItem = MapperHelper.Map<AdvertiserAccountListDto>(item);

            ValidateAdvertiser(new ValidateAdvertiserRequest { AdvertiserId = Id.Value });
            return returnItem;

        }


        public AdvertiserAccountListDto GetAccountAdvertiserInfoById(ValueMessageWrapper<int> Id)
        {
        ...
        public AdvertiserAccountSettings GetAdvertiserAccountSettings(ValueMessageWrapper<int> Id)
        {
            AdvertiserAccountSettings Settings = new AdvertiserAccountSettings();
            List<AdvertiserAccountUserDto> list = _advertiserAccountUserRepository.Query(x => x.Link.ID == Id.Value && !x.IsDeleted).Select(x => MapperHelper.Map<AdvertiserAccountUserDto>(x)).ToList();
            AdvertiserAccount AdvertiserAccountObj = _advertiserAccountRepository.Get(Id.Value);
        ...
        public ValueMessageWrapper<bool> Delete(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
        ...
        public void unArchive(ValueMessageWrapper<int> id)
        {
            AdvertiserAccount advertiserAccount = _advertiserAccountRepository.Query(x => x.ID == id.Value && x.Account.ID == Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().AccountId.Value).FirstOrDefault();
        ...
        public void ValidateAdvertiser(ValidateAdvertiserRequest request)
        {
            ...
            if (!isManager)
            {
                advertiserAccount = _advertiserAccountRepository.Query(x => x.ID == request.AdvertiserId && x.Account.ID == Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().AccountId.Value).FirstOrDefault();
                if(advertiserAccount!=null)
                advertiserAccount.Validate(Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().IsPrimaryUser == false, request.StatusCheck);
                else
                    throw new DataNotFoundException();
            }
            else
            {
                bool isLinked = _advertiserAccountRepository.Query(x => x.ID == request.AdvertiserId && !x.IsDeleted).Count() > 0;
                if (!isLinked)
                {
        ...
        public ValueMessageWrapper<bool> IsSubUserHasWriteMode(ValueMessageWrapper<int> advertiseraccId)
        {
            ...
            { return ValueMessageWrapper.Create(false); }
                var advAss = _advertiserAccountRepository.Query(x => x.ID == advertiseraccId.Value).Single();
            if (!advAss.IsRestricted)
                return ValueMessageWrapper.Create(true);
            var obj = _advertiserAccountUserRepository.Query(x => x.Link.ID == advertiseraccId.Value && x.IsDeleted == false && x.User.ID == Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().UserId.Value).FirstOrDefault();
            if (obj != null)
                return ValueMessageWrapper.Create(obj.Write);
        ...
        public ValueMessageWrapper<bool> IsSubUserHasReadMode(ValueMessageWrapper<int> advertiseraccId)
        {
            ...
            var obj = _advertiserAccountUserRepository.Query(x => x.Link.ID == advertiseraccId.Value && x.IsDeleted == false && x.User.ID == Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().UserId.Value).FirstOrDefault();
            if (obj != null)
                return ValueMessageWrapper.Create(obj.Write || obj.Read);
        ...
        {
            List<ArabyAds.AdFalcon.Domain.Model.Campaign.Campaign> Campaigns = CampaignRepository.Query(x => x.Account.ID == accountId && x.AdvertiserAccount.ID == advertiserId && x.IsDeleted == false).ToList();
        ...
        public IEnumerable<AdvertiserDto> GetByQuery(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.AdvertiserCriteria wcriteria)
        {
        ...
        public AdvertiserResult GetByQueryPagination(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.AdvertiserCriteria wcriteria)
        {
            AdvertiserCriteria criteria = new AdvertiserCriteria();
            criteria.CopyFromCommonToDomain(wcriteria);
        ...
        public ValueMessageWrapper<bool> IsReadOrWriteAdvertiserAccount(ValueMessageWrapper<int> AdvertiserAccountId)
        {
            if (OperationContext.Current.UserInfo<AdFalconUserInfo>().IsPrimaryUser)
                return ValueMessageWrapper.Create(true);

            var AdvertiserAccount = _advertiserAccountRepository.Get(AdvertiserAccountId.Value);
            int count = 0;
            if (OperationContext.Current.UserInfo<AdFalconUserInfo>().IsReadOnly)
            {

                _AdvertiserAccountReadOnlyUserRepository.Query(x => x.Link.ID == AdvertiserAccountId.Value && x.User.ID == OperationContext.Current.UserInfo<AdFalconUserInfo>().UserId && !x.IsDeleted ).Count();
                return ValueMessageWrapper.Create(count > 0);
            }
                if (!AdvertiserAccount.IsRestricted)
                return ValueMessageWrapper.Create(true);

             count = _advertiserAccountUserRepository.Query(x => x.Link.ID == AdvertiserAccountId.Value && x.User.ID == OperationContext.Current.UserInfo<AdFalconUserInfo>().UserId && !x.IsDeleted && (x.Read || x.Write)).Count();
        ...
        public AdvertiserAccountMasterAppSiteDto GetAdvertiserAccountMasterAppSiteById(ValueMessageWrapper<int> Id)
        {
            var advItem = _advertiserAccountMasterAppSiteRepository.Get(Id.Value);
        ...
        public AdvertiserAccountMasterAppSiteResultDto GetAdvertiserAccountMasterAppSite(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.AdvertiserAccountMasterAppSiteCriteria wcriteria)
        {
        ...
        public void SaveAdvertiserAccountMasterAppSite(AdvertiserAccountMasterAppSiteDto item)
        {
            ...
            ValidateAdvertiser(new ValidateAdvertiserRequest { AdvertiserId = AdvertiserAccount.Link.ID });
                          ...
                          if (RgxURL.IsMatch(ItemF) && ItemF.ToLower().IndexOf("com.") != 0)
                            {
                                if (ItemF.IndexOf("www.") == 0)
                                    ItemF = ItemF.Replace("www.","");
                                var iteminList = obj.Items.Where(M => M.AppSiteName.ToLower().Equals(ItemF.ToLower()) && M.IsDeleted == false).FirstOrDefault();
                                if (iteminList == null)
                                {
        ...
        {
            string Pattern = @"^(www.|[a-zA-Z].)[a-zA-Z0-9\-\.]+\.(com|edu|gov|mil|net|org|biz|info|name|museum|news|club|af|ax|al|dz|as|ad|ao|ai|aq|ag|ar|am|aw|ac|au|at|az|bs|bh|bd|bb|eus|by|be|bz|bj|bm|bt|bo|bq|an|nl|ba|bw|bv|br|io|vg|bn|bg|bf|mm|bi|kh|cm|ca|cv|cat|ky|cf|td|cl|cn|cx|cc|co|km|cd|cg|ck|cr|ci|hr|cu|cw|cy|cz|dk|dj|dm|do|tl|ec|eg|sv|gq|er|ee|et|eu|fk|fo|fm|fj|fi|fr|gf|pf|tf|ga|gal|gm|ge|de|gi|gr|gl|gd|gp|gu|gt|gg|gn|gw|gy|ht|hm|hn|hk|hu|is|in|id|ir|iq|ie|im|il|it|jm|jp|je|jo|kz|ke|ki|kw|kg|la|lv|lb|ls|lr|ly|li|lt|lu|mo|mk|mg|mw|my|mv|ml|mt|mh|mq|mr|mu|yt|mx|md|mc|mn|me|ms|mz|mm|na|nr|np|nl|nc|nz|ni|ne|ng|nu|nf|nc|kp|mk|mp|no|om|pk|pw|ps|pa|pg|py|pe|ph|pn|pl|pt|pr|qa|ro|ru|rw|re|bq|an|bl|gp|fr|sh|kn|lc|mf|gp|fr|pm|vc|ws|sm|st|sa|sn|rs|sc|sl|sg|bq|an|nl|sx|an|sk|si|sb|so|za|gs|kr|ss|es|lk|sd|sr|sj|sz|se|ch|sy|tw|tj|tz|th|tg|tk|to|tt|tn|tr|tm|tc|tv|ug|ua|ae|uk|us|vi|uy|uz|vu|va|ve|vn|wf|eh|ye|zm|zw)(\:[0-9]+)*(/($|[a-zA-Z0-9\.\,\;\?\'\\\+&amp;%\$#\=~_\-]+))*$";
            Regex Rgx = new Regex(Pattern, RegexOptions.Compiled | RegexOptions.IgnoreCase);
            return Rgx.IsMatch(URL);
        }
        public ValueMessageWrapper<bool> DeleteAdvertiserAccountMasterAppSite(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
                foreach (var item in Ids.Select(id => _advertiserAccountMasterAppSiteRepository.Get(id)))
                {
                    ...
                    {
                        if (item.Link != null)
                            ValidateAdvertiser(new ValidateAdvertiserRequest { AdvertiserId = item.Link.ID });
                        item.Delete();
        ...
        public ValueMessageWrapper<bool> ActivateAdvertiserAccountMasterAppSite(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
                foreach (var item in Ids.Select(id => _advertiserAccountMasterAppSiteRepository.Get(id)))
        ...
        public ValueMessageWrapper<bool> DeActivateAdvertiserAccountMasterAppSite(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
                foreach (var item in Ids.Select(id => _advertiserAccountMasterAppSiteRepository.Get(id)))
        ...
        public AdvertiserAccountMasterAppSiteItemResultDto GetAdvertiserAccountMasterAppSiteItem(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.AdvertiserAccountMasterAppSiteItemCriteria wcriteria)
        {
        ...
        public ValueMessageWrapper<bool> DeleteAdvertiserAccountMasterAppSiteItem(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
        ...
        #region AudienceList
        public AudienceSegmentResultResultDto GetAudienceSegmentsPerAdvertiser(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.AudienceSegmentCriteria wcriteria)
        {
        ...
        public ValueMessageWrapper<int> GetRootIdofFirstParty()
           {
           ...
           var root= _AudienceSegmentRepository.Query(M => M.Provider.ID == DpPartner.ID && M.Parent == null).SingleOrDefault();
            return ValueMessageWrapper.Create(root.ID);

        }

        public ValueMessageWrapper<int> GetContextualRootIdofFirstParty(string contextualFirstPartyCode)
        {
            var contPartner = _ContextualPartnerRepository.Query(M => M.Code == contextualFirstPartyCode).SingleOrDefault();
            if (contPartner != null)
            {
                var root = _ContextualSegmentRepository.Query(M => M.Provider.ID == contPartner.ID && M.Parent == null).SingleOrDefault();
               if(root!=null)
                return ValueMessageWrapper.Create(root.ID);
            }
        ...
        public void SaveAudienceSegmentPerAdvertiser(AudienceSegmentDto item)
        {
            ...
            if (audienceSegment.Advertiser != null)
                ValidateAdvertiser(new ValidateAdvertiserRequest { AdvertiserId = audienceSegment.Advertiser.ID });
            ...
            { var obj = audienceSegment;
                ...
                if (!(item.ID > 0))
                {
                    ...
                    obj.Account = new Domain.Model.Account.Account { ID = Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().AccountId.Value };
                    obj.User = new Domain.Model.Account.User { ID = Framework.OperationContext.Current.UserInfo<ArabyAds.Framework.UserInfo.IUserInfo>().UserId.Value };
        ...
        public AudienceSegmentDto GetAudienceSegmentDto(ValueMessageWrapper<int> Id)
        {
            var item = _AudienceSegmentRepository.Get(Id.Value);
        ...
        public ValueMessageWrapper<bool> DeleteAudienceSegmentPerAdvertiser
            (IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
                foreach (var item in Ids.Select(id => _AudienceSegmentRepository.Get(id)))
                {
                    ...
                    if (!item.IsDeleted)
                    {
                        if (item.Advertiser != null)
                            ValidateAdvertiser(new ValidateAdvertiserRequest { AdvertiserId = item.Advertiser.ID });
                        item.IsDeleted = true ;
        ...
        public ValueMessageWrapper<bool> DeleteAudienceSegmentPerAdvertiserForAdmin
        (IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
        ...
        public ValueMessageWrapper<bool> UnDeleteAudienceSegmentPerAdvertiser(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
                foreach (var item in Ids.Select(id => _AudienceSegmentRepository.Get(id)))
        ...
        public PixelDto GetPixelById(ValueMessageWrapper<int> Id)
        {
            var advItem = _pixelRepository.Get(Id.Value);
        ...
        public PixelResultDto GetPixel(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.PixelCriteria wcriteria)
        {
        ...
        public void SavePixel(PixelDto item)
        {
            ...
            if (Pixel.Link != null)
                ValidateAdvertiser(new ValidateAdvertiserRequest { AdvertiserId = Pixel.Link.ID });
        ...
        public ValueMessageWrapper<bool> DeletePixel(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
        ...
        public ValueMessageWrapper<bool> ActivatePixel(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
                foreach (var item in Ids.Select(id => _pixelRepository.Get(id)))
        ...
        public ValueMessageWrapper<bool> DeActivatePixel(IEnumerable<int> Ids)
        {
            if (Ids != null)
            {
                foreach (var item in Ids.Select(id => _pixelRepository.Get(id)))
        ...
        public PixelResultDto GetPixelsPerAdvertiser(ArabyAds.AdFalcon.Domain.Common.Repositories.Campaign.PixelCriteria wcriteria)
        {
            PixelCriteria criteria = new PixelCriteria();
            criteria.CopyFromCommonToDomain(wcriteria);
